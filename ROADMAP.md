...
| 0.6 | **Fix Dutch `suitable_for` / `frame_type` values in Supabase** for all ENGWE bikes (and any other Dutch-data bikes): translate `woon-werk`→`commuting`, `recreatief`→`recreation`, `transport`→`commuting`, `sportief`→`sport`, `laag-instap`/`hoog-instap`→`step-through`/`step-over`. This unlocks the commuter, recreation and other category pages for the 40+ Engwe bikes. | 🤖 | High leverage: translating these fields unlocks Engwe from a single filter (foldedSize) to all relevant category pages. |
