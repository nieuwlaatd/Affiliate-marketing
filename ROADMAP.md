...
| 0.7 | **Fix ENGWE `range_practical` km-as-miles bug in Supabase**: Several ENGWE bikes store their range in km but the field is displayed as miles (e.g. ENGWE LE20 shows "~255 mi", P275 Pro "~195 mi", P275 ST "~195 mi" -- all impossible for a single-battery e-bike in miles). Need to audit all ENGWE bikes, identify which have km values, and convert to miles (divide by 1.609). Same check for ENGWE `range_manufacturer` field. | 🤖 | Data accuracy on detail pages and on the long-range best-of page. Misleading range figures hurt trust and could cause bounce-backs from users who arrive expecting impossible range. |
...
| 2.4 | **Expand best-of coverage** (partial done 2026-06-28): ✅ fat-tire (`/best/fat-tire-ebikes` — 3 buyer's guide sections, 7 FAQs, 30+ bikes, added to sitemap) and ✅ long-range (`/best/long-range-ebikes` — 3 buyer's guide sections, 7 FAQs, bikes with 55+ mi practical range, added to sitemap). Still to do: under-$2000, step-through, off-road/SUV categories. | 🤖 | Each is a new ranking surface targeting a distinct cluster. |
