...
| 0.9 | **Data quality sweep: ~35 bikes with placeholder descriptions + zero torque/weight** (partial done 2026-07-05, continued 2026-07-05): found a broad "spec-sheet only" description problem plus `torque=0` / `weight_lbs=0` placeholder values, PLUS a distinct copy-paste bug where `weight`/`weight_lbs` had been set equal to `max_weight` (payload) or to the motor wattage number instead of the bike's actual weight. Run 1 fixed 5 bikes (3 Eunorau cargo/AWD + SAMEBIKE M20-III + Eunorau FLASH LITE ST). Run 2 (this run) fixed 8 more with verified manufacturer/retailer specs: Eunorau META20/24/26 1.0 + META275 2.0 (all 4 had `weight_lbs=286`, a straight copy of `max_weight=286` — corrected to 63.4/61.7/68.4/60 lbs respectively), Eunorau FLASH 2.0 (`weight_lbs=1000`, copied from "1000W" motor spec — corrected to 85 lbs, `max_weight` 0→440), Eunorau E-FAT-MN (weight 0→55, torque 0→80, max_weight 0→330), VTUVIA GIRAFFE (weight 0→60, torque 0→65, max_weight 0→300), plus a torque-only fix for SAMEBIKE CREST (0→85 Nm; weight not publicly documented, left as-is). All got real editorial descriptions. Remaining ~19 bikes still need the treatment: Eunorau DEFENDER, DEFENDER-S, FAT-HD 1.0/2.0, FAT-HS, FLASH AWD 1.0, FLASH LITE 2.0, META275 ST 1.0, META275 1.0, S1, SPECTER-S, SPECTER-ST, URUS; SAMEBIKE RS-A08-II, STORM, CREST (weight only); VTUVIA FMB, REINDEER 1.0; DYU C2/C5/C6 (torque not publicly documented — likely leave torque blank, could still fix weight + description). | 🤖 | Detail-page meta descriptions embed `torque` directly ("X Nm motor") — a `0` value ships a visibly broken meta description to Google and users. The weight-copy-paste bug is worse: it silently ships physically impossible bike weights (a "286 lb" 20-inch city bike, a "1000 lb" e-bike) into spec tables and meta descriptions site-wide. |
