...
| 0.9 | **Data quality sweep: ~35 bikes with placeholder descriptions + zero torque/weight** (partial done 2026-07-05): audited all brands, found no non-English text outside the already-fixed ENGWE batch, but found a broad "spec-sheet only" description problem (e.g. "DYU C6: 250W motor, 37 mi range, $819.") plus `torque=0` / `weight_lbs=0` placeholder values across mostly Eunorau (META series, FAT-HD/HS, SPECTER, FLASH, R1/R1+, Defender, URUS, S1 — ~25 bikes), a few SAMEBIKE (CREST, STORM, RS-A08-II), VTUVIA (FMB, GIRAFFE, REINDEER 1.0) and DYU (C2, C5, C6). Fixed the 5 highest-priority instances this run (see SEO-LOG 2026-07-05: the 3 Eunorau cargo/AWD bikes on `/best/cargo-ebikes`, plus SAMEBIKE M20-III and Eunorau FLASH LITE ST) using verified real-world specs from manufacturer/retailer sources. The other ~30 need the same treatment: web-search each model for real weight/torque, write a 1-2 sentence editorial description, verify before writing (do not guess unverifiable specs — DYU torque figures are not publicly published, so leave as-is rather than fabricate). | 🤖 | Detail-page meta descriptions embed `torque` directly ("X Nm motor") — a `0` value ships a visibly broken meta description to Google and users. Thin one-line descriptions are also the weakest trust signal on any page that isn't ENGWE. |
