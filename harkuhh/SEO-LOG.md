...
**GSC (28d):** 2 clicks / 1,612 impressions / 0.1% CTR, flat vs run 49 (2/1,581). Top query "electric bike for heavy riders" (2 clicks/75 impr/pos 40.1). Top pages: `/` (3 clicks/6 impr), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/28 impr/pos 9.7), `/blog/best-ebikes-for-heavy-riders` (2 clicks/437 impr/pos 30.4). No striking-distance queries, no high-impression/low-CTR pages flagged. This is the 7th consecutive run with essentially identical GSC totals and top pages -- the current top pages are at an authority ceiling (see ROADMAP next-candidate #5); further on-page work on the heavy-riders post is deprioritized pending backlinks (P4.1, Dylan-only).

**PostHog (28d):** 159 pageviews / 75 visitors, flat vs run 49. Top pages: `/e-bikes/overzicht` (21/6), `/` (17/12), `engwe-p275-se` (13/13), heavy-riders blog (13/9), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). Affiliate clicks (10 total): **Eunorau FLASH LITE ST led with 4 clicks** (a new single-bike high for any run logged so far), DYU M20 and ENGWE N1 Pro tied at 2 each, DUOTTS S26 AWD and F20 at 1 each. quiz_completed: 1.

**Action taken:** the 6-axis score-integrity sweep (value/power/comfort/range/versatility/build-quality, P0.24 + P0.51-54) is now fully closed clean across all axes as of run 49, and this run's data pull repeated run 49's signals almost exactly -- so rather than re-running an exhausted sweep, pivoted to the ROADMAP P2.3 vs-pages item. `eunorau-flash-lite-st` became this run's #1 affiliate-click bike with no comparison page. Found its natural pairing: `eunorau-flash-lite-2-0` (FLASH LITE, no "ST") is the same $1,899/92Nm/82lb/440lb-payload platform, differing only in frame geometry (step-through vs step-over) and battery configuration -- a genuine "which frame should I get" buyer query. Before shipping, checked the two bikes' displayed range side by side (75mi vs 165mi at identical battery_capacity/torque/weight) for a possible data bug; both bikes' own descriptions already document that FLASH LITE 2.0 supports two optional add-on batteries reaching a manufacturer-claimed 220mi/165mi max, while the ST is single-battery only -- confirmed legitimate, not a bug, matching the P0.53 audit's prior finding that this bike's 165mi figure is a genuine top-of-scale value. Added the pairing to `app/vs/[slug]/page.tsx`'s `MATCHUPS` and the matching sitemap entry in `app/sitemap.ts`. Verified live in dev server: page renders correctly (`/vs/eunorau-flash-lite-st-vs-eunorau-flash-lite-2-0`), all spec rows populated, zero console errors, `tsc --noEmit` clean.

**Next candidates:** (1) `dyu-m20` (2 clicks, tied for #2 this run) still has no vs-page and no same-price signal sibling found yet -- watch for one. (2) `samebike-rs-a01-pro` has dropped off the GSC top-queries list for 2 runs -- stop flagging as an active watch item. (3) Standing checks remain: price-drift (`description` vs `price`) whenever touching pricing copy; P0.16b (`eunorau-defender-s-fat-hs`) and P0.28 (`samebike-cy20-pro` torque) both still need a Dylan/human call. (4) If a future run finds both data sweeps and vs-page pairings exhausted, P3.3 (deals/price-drop page) and P4.2 (programmatic long-tail expansion) are the next unstarted 🤖-owned roadmap items.

## 2026-07-24 -- run 51

**GSC (28d):** 2 clicks / 1,612 impressions / 0.1% CTR, identical to run 50. Top query "electric bike for heavy riders" (2 clicks/75 impr/pos 40.1). Top pages: `/` (3 clicks/6 impr), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/28 impr/pos 9.7), `/blog/best-ebikes-for-heavy-riders` (2 clicks/437 impr/pos 30.4). No striking-distance queries, no high-impression/low-CTR pages flagged. 8th consecutive run with essentially identical GSC totals -- authority ceiling, not on-page (see ROADMAP next-candidate #5).

**PostHog (28d):** 159 pageviews / 75 visitors, flat vs run 50. Top pages: `/e-bikes/overzicht` (21/6), `/` (17/12), `engwe-p275-se` (13/13), heavy-riders blog (13/9), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). Affiliate clicks (10 total): Eunorau FLASH LITE ST still leads with 4 clicks, DYU M20 and ENGWE N1 Pro tied at 2 each (DYU M20's 2nd consecutive run at this signal level, per run 50's flagged "watch for a sibling" candidate), DUOTTS S26 AWD and F20 at 1 each. quiz_completed: 1.

**Action taken:** followed run 50's queued next candidate -- paired `dyu-m20` (tied for #2 affiliate-click bike, 2 consecutive runs, still no vs-page) with `dyu-c9`, DYU's only other bike at the identical $899 price point but a genuinely different build (M20: 88 lb full-suspension fat-tire commuter, 18 Ah battery, 50 Nm; C9: 66 lb lighter folding tourer, 16 Ah battery, 45 Nm, longer manufacturer range) -- a real "same price, which DYU" query. Added the pairing to `app/vs/[slug]/page.tsx`'s `MATCHUPS` and the matching sitemap entry in `app/sitemap.ts`. While sourcing the comparison, found a new bug class: `dyu-c9`'s description claimed its 48V 15.6Ah battery "is the largest in the DYU lineup" while `dyu-m20`'s description made the identical claim for its own 18.2Ah pack -- two bikes' free-text descriptions directly contradicting each other, a first instance of this bug appearing as a cross-bike contradiction rather than a description contradicting its own spec table. Verified via the `battery_capacity` column across all 10 DYU bikes: M20 (18 Ah) genuinely beats C9 (16 Ah), so C9's claim was wrong. Fixed C9's description to "second-largest in the DYU lineup (behind only the M20's 18.2Ah pack)". Swept the same "largest/highest in lineup" superlative pattern sitewide on battery and torque claims: DUOTTS F20 ("one of the largest packs"), Walfisk ET-7 Ultra ("the largest pack in our catalog" -- verified true, 45 Ah is the genuine sitewide max), VTUVIA CMB ("175 Nm, the highest torque in VTUVIA's catalog" -- verified true against `vtuvia-fmb`'s 160 Nm), and DUOTTS F26 Lite ("highest single-motor torque in DUOTTS' fat-tire lineup") all checked clean against the actual data. Also re-ran the standing P0.30 price-drift sweep (`description ~ '\$[0-9]'` vs `price`) across all 61 rows with embedded dollar figures -- zero drift found, still clean. Verified live in dev server: `/vs/dyu-m20-vs-dyu-c9` renders correctly (all spec rows, price both $899, overall score 7.4 vs 7.2), `/e-bikes/dyu/dyu-c9` shows the corrected description, zero console errors, `tsc --noEmit` clean.

**Next candidates:** (1) Sweep other superlative phrasing sitewide ("lightest", "heaviest", "most powerful", "fastest") for the same dueling-claim bug class found this run on battery superlatives -- only battery and torque claims were checked so far. (2) `samebike-rs-a01-pro` has now dropped off the GSC top-queries list for 3 runs -- stop flagging as an active watch item. (3) Standing checks remain: price-drift (`description` vs `price`) whenever touching pricing copy (ran clean again this run); P0.16b (`eunorau-defender-s-fat-hs`) and P0.28 (`samebike-cy20-pro` torque) both still need a Dylan/human call. (4) P3.3 (deals/price-drop page) is blocked on missing original-price/MSRP data in the catalog -- do not build without a real discount source; flag to Dylan if one becomes available. (5) P4.2 (programmatic long-tail expansion) remains the next unstarted 🤖-owned roadmap item if data sweeps and vs-page pairings both come up empty in a future run.

## 2026-07-24 (run 52) — Superlative-claim sweep closes P0.55's queued follow-up; one real bug found

**GSC snapshot (28d):** 2 clicks / 1,612 impressions, CTR 0.1% — flat for a 9th+ consecutive run, same top query ("electric bike for heavy riders", 2 clicks/75 impr/pos 40.1) and same top pages. One notable shift: `samebike-rs-a01-pro` reappeared in the top-pages list (2 clicks/28 impr, pos 9.7) after being absent for several runs — reverses the "stop treating as active" note from run 51's next-candidates list.

**PostHog snapshot (28d):** 159 pageviews / 75 visitors, identical to the last several runs. 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26 1, DUOTTS F20 1). No new signal bikes.

**Action taken:** with no new demand or behavior signal, followed run 51's queued next-candidate #3 (sweep other superlative phrasing — "lightest/heaviest/most powerful/fastest" — sitewide for the same dueling-claim bug class found on batteries in P0.55). Queried every `description` matching lightest/heaviest/most powerful/fastest/highest/biggest/longest/largest superlative patterns (13 rows across DUOTTS, DYU, ENGWE, Eunorau, VTUVIA, Walfisk) and checked each against the actual DB values for its stated scope (brand lineup, catalog-wide, or category-specific):

- **Bug found and fixed:** `dyu-stroll-1` claimed "43 lb weight, the lightest in the DYU lineup" — but `dyu-a5` (40 lbs) and `dyu-d3f` (41.9 lbs) are both lighter, and neither makes a competing claim (a one-sided false superlative, not a dueling pair like P0.55). Rewrote to "lighter than any of DYU's fat-tire models" — verified true against all 6 DYU fat-tire/city models (C2 52 lbs, C5 59.5 lbs, C6 60 lbs, T1 50.7 lbs, M20 88.2 lbs, C9 66.1 lbs), and left A5/D3F's compact-folder category out of the comparison since they're a genuinely different form factor, not a rival claim to arbitrate.
- **Verified clean (no changes):** `duotts-duotts-f26lite-electric-bike` ("highest single-motor torque in DUOTTS' fat-tire lineup", 80 Nm) — confirmed DUOTTS' dual-motor fat-tire bikes (F26/N26/S26, 110-130 Nm) are combined-torque figures, not single-motor, and E26 (also 80 Nm) is categorized as an e-MTB, not fat-tire (no fat-tire tire spec), so F26 Lite's single-motor fat-tire claim holds. `engwe-l20-3-0-pro` ("100 Nm, the highest in the ENGWE lineup") — confirmed against all 26 ENGWE torque values, genuinely the max. `vtuvia-cmb` (175 Nm) and `walfisk-...-et-7-...` (130 Nm / 45 Ah) were already verified true in the P0.55 sweep; re-confirmed. Hedged claims ("among the longest", "one of the largest") on `duotts-f20`, `engwe-p275-pro`, `eunorau-flash-lite-2-0` left as-is — accurate qualifying language, not absolute claims.
- **Standing check re-run clean:** `description ~ '\$[0-9]'` vs `price` (P0.30 bug class) — all 61 rows with embedded dollar figures match their `price` column exactly, including cross-referenced sibling-delta math (e.g. "$150 more than the F26" checks out against both rows' actual prices). Zero drift found.

Verified live in dev server (`/e-bikes/dyu/dyu-stroll-1`): description, highlights, and spec table all consistent, zero console errors. `tsc --noEmit` clean (DB-only run, no code touched).

**Next candidates:** (1) GSC/PostHog have now been flat for 9+ consecutive runs on the same signal set — `samebike-rs-a01-pro`'s reappearance this run is the only new movement; watch for a second consecutive run before treating it as active again. (2) The superlative-claim and price-drift sweeps are now both clean across the full catalog — next sweep-pattern candidate would be checking "among the X" hedged claims for accuracy too (not just absolute superlatives), or extending the sweep to best-of page copy and blog posts (only bike `description` fields have been checked so far). (3) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) both still need a Dylan/human call. (4) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (5) P3.3 (deals page) still blocked on a real discount/MSRP data field.

## 2026-07-25 (run 53) -- False superlative fixed on hills blog post + new DUOTTS vs-page

**GSC snapshot (28d):** 2 clicks / 1,628 impressions, CTR 0.1% -- flat for a 10th+ consecutive run, same top query ("electric bike for heavy riders", 2 clicks/75 impr/pos 40.0) and same top pages (`/`, `samebike-rs-a01-pro` at 2 clicks/30 impr/pos 9.7, heavy-riders blog at 2 clicks/455 impr/pos 30.1, `/best/folding-ebikes`, `duotts-duotts-c29-k`, `duotts-f20`, `duotts-s26`, `eunorau-meta-24-1`, `samebike-ebe2`, Walfisk ET-7). `samebike-rs-a01-pro` appeared in top pages for a 2nd consecutive run (per run 52's "watch for a second run" flag) -- but it already has full P1.1 editorial depth (run 6) and its own vs-page (vs `samebike-rs-a01-men`), so there is no further on-page lever here; treating it as confirmed-stable, not a new action item. No striking-distance queries, no high-impression/low-CTR pages flagged.

**PostHog snapshot (28d):** 159 pageviews / 75 visitors, identical to the last several runs. Top pages: `/e-bikes/overzicht` (21/6), `/` (17/12), `engwe-p275-se` (13/13), heavy-riders blog (13/9), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26 AWD 1, DUOTTS F20 1). 1 `quiz_completed`. No new signal bikes.

**Action taken:** with both data sources flat again and the bike-description superlative/price-drift sweeps already exhaustively covered, followed run 52's queued next-candidate #2: extended the "lightest/heaviest/most powerful/fastest/highest/biggest/longest/largest" superlative sweep beyond bike `description` fields into best-of page copy (`app/best/[category]/page.tsx`) and blog posts (`lib/blog-data.ts`), which no prior sweep had checked.

- **Best-of page copy:** grep for the same superlative pattern found no per-bike absolute claims -- all matches were generic price-tier/buying-advice language ("battery quality is the single biggest variable...", "fastest legal option" referring to Class 3 speed limits generically), not falsifiable per-bike claims. Clean.
- **Bug found and fixed:** `/blog/best-ebikes-for-hills`'s "Best for power and range on hills: Eunorau FLASH" section claimed "At 220 Nm from a single rear hub, the Eunorau FLASH... is the most powerful single-motor bike in our catalog." Ran a sitewide `torque`/`motor_type` sweep and found this false: Eunorau's own R1 (330 Nm) and R1+ (500 Nm) are both single-motor bikes (mid-drive) that exceed 220 Nm -- the post's claim had no motor-architecture qualifier, so it read as an absolute, falsifiable statement. Verified 220 Nm genuinely is the sitewide max among non-AWD **rear-hub** motors specifically (next highest rear-hub figures top out around 160-184 Nm once AWD-combined figures are excluded). Rescoped the claim to "the most powerful single-motor rear-hub bike in our catalog" and added a parenthetical noting the mid-drive R1/R1+ exceed it at nearly double the price -- same scoping precedent as P0.56's F26 Lite fat-tire-vs-e-MTB distinction (a true claim within an honestly-stated category beats a false unqualified one). Bumped the post's `updatedAt` to 2026-07-25.
- **Also checked while sweeping:** the same post's Defender-S pick ("the strongest hill climber in our catalog for everyday riders") is already correctly hedged with "for everyday riders," excluding the R1/R1+ flagships by design -- left as-is, not a bug.

**Second item -- vs-page for this run's converter:** `duotts-f20` logged a confirmed affiliate click this run with no comparison page (the same gap pattern that prompted the FLASH LITE ST and DYU M20 vs-pages in runs 50-51). Checked all DUOTTS siblings by price/spec and found F20's natural pairing: `duotts-duotts-f26lite-electric-bike` (F26 Lite), DUOTTS' other "F-series" full-suspension fat-tire bike at a close price ($1,099 vs $1,199) with a genuine tradeoff (F20: 87mi manufacturer range / 70 Nm vs F26 Lite: 50mi manufacturer range / 80 Nm) -- a real "which F-series DUOTTS" query, not an arbitrary pairing. Added to `app/vs/[slug]/page.tsx`'s `MATCHUPS` and the matching sitemap entry in `app/sitemap.ts`.

**Verified:** live in dev server -- `/vs/duotts-f20-vs-duotts-duotts-f26lite-electric-bike` renders correctly (all spec rows populated, F20 wins overall 8.0 vs 6.7, price/motor/battery/weight/payload/warranty all correct), `/blog/best-ebikes-for-hills` shows the corrected claim and updated date, zero console errors on either page. `tsc --noEmit` clean.

**Expected impact:** closes the P0.55/P0.56 superlative-sweep initiative's last open scope gap (best-of + blog copy, not just bike descriptions) with one real catch -- a false "most powerful" claim in a buying-advice post is exactly the kind of easily fact-checked statement that undermines trust when a reader cross-references it against another page's spec table. The new vs-page gives this run's only fresh converter (DUOTTS F20) a comparison surface, following the established "give converters a vs-page when they don't have one" pattern.

**Next candidates:** (1) GSC/PostHog flat for 10+ consecutive runs -- data-quality, superlative, and price-drift sweeps are now exhausted across bike descriptions, best-of pages, and blog posts. Next sweep-pattern candidate: hedged claims ("among the longest", "one of the largest") haven't been individually fact-checked, only absolute superlatives have. (2) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) both still need a Dylan/human call. (3) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (4) If the next run's data pull is flat again and no new sweep angle surfaces, P4.2 (programmatic long-tail expansion) is the next unstarted 🤖-owned roadmap item worth a dedicated pass.

---

## Run 54 (2026-07-25)

**GSC signals:** Flat for an 11th consecutive run -- 2 clicks / 1,628 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/75 impr/pos 40.0). Top pages unchanged: `/blog/best-ebikes-for-heavy-riders` (2 clicks/455 impr/pos 30.1), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), plus single-click pages across DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 159 pageviews / 75 visitors (28-day window), matching the last several runs. `engwe-p275-se` tied for the #1 page this run (13 views/13 visitors/13 sessions, level with `/e-bikes/overzicht`) -- a repeat of the surge first seen in run 39, and still with no comparison surface. 10 total `affiliate_link_clicked` events; top converters unchanged (FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26/F20 1 each).

**Work this run (2 items):**

1. **Hedged-claim sweep, closed clean sitewide (P0.58).** Followed run 53's queued next candidate: fact-checked every bike `description` using softer superlative phrasing ("among the largest/longest/lightest/heaviest/highest", "one of the ...") -- distinct from the absolute "the most/largest" claims P0.55-0.57 already covered. Found 9 matching bikes (DUOTTS F20; ENGWE M1, N1 Air, P275 Pro; Eunorau FLASH LITE, FLASH LITE ST; Walfisk ET-7 Ultra, WF750 UrbanX). Verified each against a full sitewide or brand-scoped ranking query (weight, max_weight, torque, battery_capacity, range). Every claim held up: e.g. ET-7 Ultra's 130 Nm is genuinely Walfisk's max by a wide margin (WF750/WF26 both 80 Nm); N1 Air's 34.4 lbs is the single lightest of 26 ENGWE bikes; WF750 UrbanX's 35 Ah battery is the 2nd-largest of 37 bikes in its $1,300-1,700 price band; M1's 441 lb payload is tied for the sitewide max. Zero false claims found -- this closes the hedged-claim sub-pattern with a clean bill of health (same "clean audit" outcome as P0.54's versatility-score sweep).

2. **New vs-page: `engwe-p275-se` vs `engwe-p275-pro` (P0.59).** PostHog's #1-tied page this run had no comparison surface. Paired it with its natural same-product-line sibling: P275 SE ($899, step-through, rear-hub, torque sensor) vs P275 Pro ($1,099, step-over, mid-drive, Gates carbon belt drive) -- a real "which ENGWE P275 should I buy" query given both share the product name. Added to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts` (the second list is easy to miss -- caught it this run before shipping). Verified live in dev server: `/vs/engwe-p275-se-vs-engwe-p275-pro` renders all spec rows correctly (price, overall score 7.8/8.5, range 46mi/121mi, torque 55/65 Nm, battery 13/19 Ah, weight, payload), zero console errors. 13 vs-pages total now (5 brand-level + 8 bike-level).

**Verified:** `tsc --noEmit` clean after both changes (sitemap.ts + vs page.tsx edits). Both pages checked live in a local dev server.

**Expected impact:** the hedged-claim sweep exhausts the last open superlative sub-pattern flagged since P0.55, giving the site's "How We Test" trust promise a clean bill of health across absolute superlatives, hedged superlatives, and price-drift. The new vs-page gives this run's strongest traffic signal (P275 SE) a bottom-of-funnel comparison surface exactly when it's showing real repeat visits.

**Next candidates:** (1) GSC/PostHog flat for 11+ consecutive runs; every data-quality, score-integrity, and superlative-claim sweep pattern identified so far is now closed clean or fixed. (2) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) both still need a Dylan/human call. (3) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (4) With sweep-pattern angles exhausted, P4.2 (programmatic long-tail expansion) is the clearest next unstarted 🤖-owned roadmap item -- worth scoping a concrete first slice next run (e.g. sibling-pair vs-pages within a brand that still lack one, or a location x category cross) rather than a large undifferentiated build.

## Run 55 (2026-07-25)

**GSC signals:** Flat for a 12th consecutive run -- 2 clicks / 1,628 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/75 impr/pos 40.0). Top pages unchanged: `/blog/best-ebikes-for-heavy-riders` (2 clicks/455 impr/pos 30.1), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), plus single-click pages across DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 160 pageviews / 76 visitors (28-day window), essentially flat vs run 54 (159/75). Top pages: `/e-bikes/overzicht` (21/6), `/` (18/13), `engwe-p275-se` (13/13), heavy-riders blog (13/9), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26 AWD 1, DUOTTS F20 1). No new signal bikes.

**Work this run:** with both data sources flat for the 12th straight run and every data-quality/score-integrity/superlative sweep already closed clean (per run 54's own conclusion), followed run 54's queued next candidate: started P4.2 (programmatic long-tail expansion), the last unstarted 🤖-owned roadmap item. Scoped the first slice as run 54 suggested -- comparison pages for sibling models within a brand that lack one yet -- but chose *naming-confusion* pairs (same model-name stem, real spec gap) over signal-driven pairs, since every current signal bike already has a vs-page. Queried the full 109-bike catalog (`brand, model, price, torque, range, weight` via Supabase) for same-brand pairs sharing a name root with a genuine spec difference and no existing MATCHUPS entry:

- `samebike-m20` vs `samebike-m20-iii` -- same base name, but M20-III is a real generational jump ($1,299/100Nm/87mi manufacturer range vs $1,599/160Nm/174mi, nearly double).
- `engwe-l20` vs `engwe-l20-boost` -- ENGWE's L20 line has 4 similarly-named SKUs (L20, L20 Boost, L20 3.0 Boost, L20 3.0 Pro); found a genuinely counterintuitive pairing worth surfacing directly: L20 Boost is $250 *cheaper* than the base L20 while carrying *more* torque (75 vs 50 Nm) and a higher payload (331 vs 265 lbs) -- a real trap for anyone assuming "Boost" costs more.
- `samebike-rs-a02-pro` vs `samebike-rs-a02-plus` -- same chassis/weight/payload, Plus is a clear torque/range upgrade ($959/80Nm/68mi vs $1,399/100Nm/81mi manufacturer range) for $440 more, a genuine "is the Plus worth the extra $440" query.

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together this time, avoiding the near-miss run 54 flagged about the sitemap list being easy to forget).

**Verified:** `tsc --noEmit` clean. All 3 pages checked live in a local dev server (`/vs/samebike-m20-vs-samebike-m20-iii`, `/vs/engwe-l20-vs-engwe-l20-boost`, `/vs/samebike-rs-a02-pro-vs-samebike-rs-a02-plus`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted, zero console errors on any of the three.

**Expected impact:** opens the first slice of P4.2, the last unstarted 🤖-owned roadmap item, with 3 new bottom-of-funnel comparison pages (16 vs-pages total now, 5 brand-level + 11 bike-level) targeting real "which SKU should I buy" queries within a brand's own lineup -- a query pattern the site had zero coverage of until this run (every prior vs-page paired either two different brands or a GSC/PostHog-signal bike with its closest sibling, never two SKUs chosen purely for a naming-confusion story).

**Next candidates:** (1) GSC/PostHog flat for 12+ consecutive runs; continue leaning on P4.2 slices while watching for a new signal. (2) More within-brand naming-confusion candidates for the next pass: DUOTTS N26 vs F26 (near-identical price/torque, N26 has full suspension vs F26 front-only -- a real differentiator), ENGWE L20 3.0 Boost vs L20 3.0 Pro (the other half of the confusing L20 family), VTUVIA SX20 vs SN100 (near-identical specs -- worth a closer data check for a genuine differentiator before pairing, don't force a comparison with no story), SAMEBIKE CREST vs STORM (byte-identical spec columns in the DB -- would need a samebike.com verification pass to confirm a real differentiator exists, same caution as the P0.35 20LVXD30-II/CY20 case, before shipping a comparison page). (3) A location x category cross ("best commuter e-bikes in [state]") remains a bigger, un-scoped P4.2 slice worth a dedicated future run rather than folding into a data-sweep run. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy; `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) both still need a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 56 (2026-07-25)

**GSC signals:** Flat for a 13th consecutive run -- 2 clicks / 1,628 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/75 impr/pos 40.0). Top pages unchanged: `/blog/best-ebikes-for-heavy-riders` (2 clicks/455 impr/pos 30.1), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), plus single-click pages across DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 162 pageviews / 78 visitors (28-day window), essentially flat vs run 55 (160/76). Top pages unchanged: `/e-bikes/overzicht` (21/6), `/` (19/14), `engwe-p275-se` (13/13), heavy-riders blog (13/9), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26 AWD 1, DUOTTS F20 1). No new signal bikes.

**Work this run:** with both data sources flat for the 13th straight run and no new signal bikes, followed run 55's queued next-candidate #2 exactly: worked the second slice of P4.2 naming-confusion vs-pages from the candidate list it left behind.

- Pulled fresh specs via Supabase for the 4 queued candidates (`duotts-f26`/`duotts-n26`, `engwe-l20-3-0-boost`/`engwe-l20-3-0-pro`, `vtuvia-sx20`/`vtuvia-sn100`, `samebike-crest`/`samebike-storm`) to confirm each pair has a real story before shipping.
- **Shipped 3:** `duotts-f26-vs-duotts-n26` (F26 $1,349/front suspension vs N26 $1,299/full suspension -- N26 is cheaper *and* has more suspension, a genuine "why pay more for less" trap); `engwe-l20-3-0-boost-vs-engwe-l20-3-0-pro` (Boost $1,399/75Nm/63mi practical vs Pro $1,699/100Nm/75mi practical -- a real "is the $300 upgrade worth it" query, the other half of the L20 family after run 55's L20-vs-L20-Boost page); `vtuvia-sx20-vs-vtuvia-sn100` (identical $1,599 price, torque, battery and range, but SX20 is a 20in step-through rated to 330 lb payload vs SN100's 26in step-over rated to 400 lb -- same platform, different frame geometry and payload ceiling).
- **Skipped 1, again:** `samebike-crest`/`samebike-storm` re-confirmed byte-identical across every spec column (price $1,549, torque 85, weight 99 lbs, max_weight 330, range 87/43mi, battery 20Ah) -- same finding as run 55. Left unpaired rather than shipping a comparison page with no real differentiator to report; still needs a samebike.com sourcing pass to determine whether these are genuinely different products (per the P0.35 20LVXD30-II/CY20 precedent) or a duplicate-row data bug.
- Both spec sources (DUOTTS F26/N26 torque+suspension via P0.31, VTUVIA SX20/SN100 wheel size+frame type via the P0.36 stub-description batch) were already web-verified in prior runs, so no new sourcing was needed for the 3 shipped pairs -- only fresh DB reads to confirm the numbers still hold.

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per run 55's established practice).

**Verified:** `tsc --noEmit` clean (silent = pass). All 3 pages checked live in a local dev server (`/vs/duotts-f26-vs-duotts-n26`, `/vs/engwe-l20-3-0-boost-vs-engwe-l20-3-0-pro`, `/vs/vtuvia-sx20-vs-vtuvia-sn100`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, zero console errors on any of the three.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (19 vs-pages total now, 5 brand-level + 14 bike-level), each targeting a real "which SKU" query with a genuine spec-driven story rather than an arbitrary pairing. Declining to force the CREST/STORM pairing a second time keeps the standing bar (every vs-page needs a verified, real differentiator) intact rather than shipping a comparison page that would just restate identical numbers twice.

**Next candidates:** (1) GSC/PostHog flat for 13+ consecutive runs; continue leaning on P4.2 slices while watching for a new signal. (2) `samebike-crest`/`samebike-storm` still need a dedicated samebike.com sourcing pass to resolve whether they're genuinely different products or a duplicate-row bug (2 consecutive runs deferred, same category as P0.16b/P0.28 -- may be worth treating as a standing Dylan-flag rather than re-checking every run). (3) A location x category cross ("best commuter e-bikes in [state]") remains a bigger, un-scoped P4.2 slice worth a dedicated future run. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy; `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) both still need a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 57 (2026-07-26)

**GSC signals:** Flat for a 14th consecutive run -- 2 clicks / 1,648 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/78 impr/pos 40.0). Top pages unchanged: `/` (3 clicks/6 impr), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), `/blog/best-ebikes-for-heavy-riders` (2 clicks/463 impr/pos 30.1), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 169 pageviews / 82 visitors (28-day window), essentially flat vs run 56 (162/78). Top pages unchanged: `/e-bikes/overzicht` (21/6), `/` (20/15), heavy-riders blog (16/11), `engwe-p275-se` (13/13), `duotts-duotts-c29-k` (11/7), `duotts-s26` (10/9). 11 `affiliate_link_clicked` events, same converter set with DUOTTS S26 AWD now up to 2 clicks (from 1 last run) -- otherwise unchanged (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS F20 1). 1 `quiz_completed`. No new signal bikes.

**Action taken:** with both data sources flat for the 14th straight run and no new signal bikes, followed run 56's queued next-candidate #2 exactly: resolved the `samebike-crest`/`samebike-storm` duplicate-spec question that had been deferred twice (runs 55-56) rather than guessed at.

- Confirmed via Supabase both rows remain byte-identical (price $1,549, torque 85 Nm, weight 99 lbs, range 87/43mi, battery 20Ah, `frame_type='sport'` on both).
- Found SAMEBIKE's own official "Storm vs. Crest" comparison page (samebike.com/blogs/ebikes/storm-vs-crest-fat-tire-electric-mountain-bike) via WebSearch/WebFetch -- confirms these are two genuinely different products sharing the same 85+ Nm/48V 20Ah/26x4.0" fat-tire platform, differing in frame: STORM is a traditional high top-tube step-over frame ("sporty handling & high rigidity" for technical descents), CREST drops the top tube for a low-step step-through design ("quick mounting & open comfort" for loaded cargo or bulky winter layers).
- Fixed `frame_type` in Supabase: STORM -> `step-over`, CREST -> `step-through` (both were the non-differentiating placeholder `sport`). Rewrote both descriptions and highlights to state the frame difference explicitly instead of reading as near-duplicates.
- Added the pairing to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per the established practice) -- a genuine "which frame fits me" query now that the two bikes have a real, sourced differentiator.
- Verified live in dev server: `/vs/samebike-crest-fat-tire-mountain-e-bike-vs-samebike-storm-fat-tire-mountain-e-bike` renders all spec rows correctly (identical price/motor/battery/range/weight/payload, both score 6.9), zero console errors; `/e-bikes/samebike/samebike-crest-fat-tire-mountain-e-bike` shows the corrected description, highlights, and "Frame type: Step-through" in the spec table, zero console errors. `tsc --noEmit` clean.

**Verified:** `tsc --noEmit` clean after the sitemap.ts + vs page.tsx + Supabase edits. Both the vs-page and the CREST detail page checked live in a local dev server.

**Expected impact:** closes the last standing "needs a Dylan/human call" item that had a real, sourceable answer (unlike P0.16b/P0.28, which remain genuinely ambiguous), fixes a `frame_type='sport'` placeholder that had silently excluded CREST from `/best/step-through-ebikes` since the row was created, and adds a 20th vs-page (6th within-brand naming-confusion pairing) targeting a real "which frame fits me" query.

**Next candidates:** (1) GSC/PostHog flat for 14+ consecutive runs; continue leaning on P4.2 slices (location x category cross remains the biggest unscoped slice) while watching for a new signal -- DUOTTS S26 AWD ticking up to 2 affiliate clicks this run is worth a second-run confirmation before treating as a new signal bike. (2) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call (no official source resolves either, unlike CREST/STORM). (3) A location x category cross ("best commuter e-bikes in [state]") remains a bigger, un-scoped P4.2 slice worth a dedicated future run rather than folding into a data-sweep run. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 58 (2026-07-26)

**GSC signals:** Flat for a 15th consecutive run -- 2 clicks / 1,648 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/78 impr/pos 40.0). Top pages unchanged: `/` (3 clicks/6 impr), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), `/blog/best-ebikes-for-heavy-riders` (2 clicks/463 impr/pos 30.1), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 169 pageviews / 82 visitors (28-day window), essentially flat vs run 57 (same totals). Top pages unchanged: `/e-bikes/overzicht` (21/6), `/` (20/15), heavy-riders blog (16/11), `engwe-p275-se` (13/13), `duotts-duotts-c29-k` (11/7), `duotts-s26` (10/9). 11 `affiliate_link_clicked` events -- same converter set with DUOTTS S26 AWD holding at 2 clicks for a 2nd consecutive run (was 1 in run 56, 2 in run 57, 2 again this run) -- but S26 already has a comparison surface (`vs/engwe-n1-pro-vs-duotts-s26`, added run 50) and full P1.1 depth (run 10), so no further action item there. Otherwise unchanged (Eunorau FLASH LITE ST 4, DYU M20 2, ENGWE N1 Pro 2, DUOTTS F20 1). 1 `quiz_completed`. No new signal bikes.

**Action taken:** with both data sources flat for the 15th straight run and every data-quality/score-integrity/superlative sweep already closed clean, followed the queued next-candidate #1 and #3: took a first real crack at P4.2's location x category cross, the biggest unscoped roadmap slice, deferred for several runs specifically because it needed a "dedicated future run" rather than a quick pass.

Considered the straightforward approach first -- a new `/best-ebikes/[state]/[category]` route generating up to 50 states x 7 categories (350 pages) -- and rejected it: most combinations would just be a templated re-shuffle of the same 9-bike top list with no genuine per-combination differentiation, which is exactly the kind of thin/near-duplicate programmatic content Google's Helpful Content system penalizes, and it would multiply the sitemap without multiplying real value. Instead scoped a smaller, higher-confidence slice that ships real location x category relevance on pages that are *already* indexed (zero new URLs, zero duplicate-content risk):

- Added `getStateTerrainCategory()` to `lib/state-data.ts` -- classifies each state's existing, already-accurate `terrain` free-text (itself the product of the run 16-18 legal/terrain verification passes) into `hilly` / `flat` / `mixed` via keyword matching (mountain/steep/Rockies/Appalachian/elevation/glacier/fjord/sawtooth -> hilly signal; flat/plains/prairie -> flat signal; both-or-neither -> `mixed`, a deliberately conservative fallback so genuinely diverse states like California or Colorado don't get a one-sided list).
- Modified `app/best-ebikes/[state]/page.tsx`'s "Top-Rated E-Bikes" bike selection: `hilly` states now sort by `scoreOverall + (mid-drive ? 1.0 : 0) + torque/300` (a first draft weighted torque/100, which let two $4,299-4,499 niche flagships -- Eunorau R1/R1+, 330-500 Nm -- dominate every single hilly state's top slot regardless of overall score; caught this live in `/best-ebikes/vermont` during verification and rebalanced the formula so `scoreOverall` stays the dominant term and torque is a modest tiebreak, confirmed L20 3.0 Pro at 8.7 now correctly leads over R1+ at 6.5). `flat` states sort by `scoreOverall + rangePractical/100` (verified on `/best-ebikes/texas`: FLASH 165mi and P275 ST/Pro 121mi now lead). `mixed` states are unchanged (original `scoreOverall` sort, no new copy) -- verified on `/best-ebikes/california`.
- Added a one-line contextual note + internal link above the bike grid on hilly/flat state pages only: hilly links to `/blog/best-ebikes-for-hills`, flat links to `/best/long-range-ebikes`. Mixed states keep the original generic copy, unchanged.
- Ran the classifier against all 50 states as a sanity check before shipping: 5 hilly (Oregon, Utah, West Virginia, Maine, Vermont), 20 flat (Texas, Florida, Washington, Illinois, Georgia, Michigan, New Jersey, Minnesota, Ohio, Missouri, Wisconsin, Indiana, Louisiana, Iowa, Kansas, Mississippi, Nebraska, North Dakota, Delaware, Rhode Island), 25 mixed (including Colorado, Wyoming, Idaho, New Mexico, Montana, Arizona -- all genuinely diverse-terrain states whose text mentions both flat and mountainous regions, correctly falling back to the safe default rather than a forced one-sided classification).

**Verified:** `tsc --noEmit` clean. Live in dev server on `/best-ebikes/vermont` (hilly -- L20 3.0 Pro/N1 Pro lead, hill-climbing guide link renders), `/best-ebikes/texas` (flat -- FLASH/P275 ST/Pro lead, long-range guide link renders), `/best-ebikes/california` (mixed -- original generic copy, no terrain note, confirming the fallback path is unchanged), zero console errors on any of the three.

**Expected impact:** operationalizes the "location x category" theme on all 50 already-indexed, already-legally-verified state pages without adding a single new URL or duplicate-content page -- a state page now surfaces genuinely more-relevant picks (and a matching internal link) based on its own real terrain data instead of one identical top-9-by-score list everywhere. Leaves the bigger "new page per state x category combo" version of P4.2 explicitly unscoped rather than shipping it blind.

**Next candidates:** (1) GSC/PostHog flat for 15+ consecutive runs; DUOTTS S26 AWD has now held at 2 affiliate clicks for 2 straight runs but already has full depth + a vs-page, so no further action needed there specifically -- keep watching for a genuinely new signal bike instead. (2) The bigger P4.2 location x category page-matrix (new URLs, not a re-sort of existing pages) is still unscoped -- if attempted, do it selectively (e.g. only for state x category pairs with a real, sourceable local angle) rather than a blind 350-combo generation. (3) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 59 (2026-07-26)

**GSC signals:** Flat for a 16th consecutive run -- 2 clicks / 1,648 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/78 impr/pos 40.0). Top pages unchanged: `/` (3 clicks/6 impr), `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/30 impr/pos 9.7), `/blog/best-ebikes-for-heavy-riders` (2 clicks/463 impr/pos 30.1), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 171 pageviews / 83 visitors (28-day window), essentially flat vs run 58 (169/82). Top pages unchanged: `/e-bikes/overzicht` (21/6), `/` (20/15), heavy-riders blog (17/12), `engwe-p275-se` (13/13), `duotts-duotts-c29-k` (11/7), `duotts-s26` (10/9). 12 `affiliate_link_clicked` events (up from 11) -- Eunorau FLASH LITE ST ticked up to 5 clicks (from 4), still the clear top converter; otherwise the same set (DYU M20 2, ENGWE N1 Pro 2, DUOTTS S26 AWD 2, DUOTTS F20 1). 1 `quiz_completed`. No new signal bikes.

**Action taken:** with both data sources flat for the 16th straight run and every standing data-quality/score-integrity/superlative/price-drift sweep already closed clean (re-verified both this run: a sitewide `description ~ '\$[0-9]'` vs `price` check across all 61 dollar-mentioning rows found zero drift, and a `length(description) < 220` sweep found only the one already-flagged, genuinely ambiguous `eunorau-defender-s-fat-hs` stub remaining -- P0.16b, still needs a Dylan call, not a guess), continued P4.2's within-brand naming-confusion vs-page slice (3rd installment after runs 55-57).

Mined the catalog for remaining same-name-stem sibling pairs with a real, already-verified spec differentiator (no new web research needed -- all specs below were already sourced from official manufacturer pages in prior runs: P0.33 for SAMEBIKE LO26, P0.36 for VTUVIA Zeal, standard DB data for DUOTTS C29). Pulled fresh specs via Supabase to confirm each pair before shipping:

- **`duotts-c29-vs-duotts-duotts-c29max-electric-bike`** ("C29 vs C29Max"): same 750W rear-hub platform, but C29Max adds a torque sensor and 10 more Nm (65 -> 75 Nm) while trading 2mi of practical range and costing $350 more ($799 -> $1,149) -- a real "is the torque sensor worth it" question within DUOTTS' own C29 family.
- **`vtuvia-zeal-lt7-vs-vtuvia-zeal-xt8`** ("Zeal LT7 vs Zeal XT8"): same step-through rear-hub chassis, XT8 is a genuine $500 upgrade (85 Nm/52mi practical vs LT7's 65 Nm/45mi, both 350 lb payload) -- a real "which Zeal" query given both share the product-line name.
- **`samebike-lo26-plus-vs-samebike-lo26-ii-yd`** ("LO26 Plus vs LO26-II-YD"): both full-suspension step-over 75 Nm fat-tire bikes sharing the LO26 name stem, but LO26-II-YD is $100 cheaper with a smaller payload rating (264 vs 330 lb) and shorter practical range (40mi vs 44mi) -- LO26-II-YD's own description already references "sits below its LO26 Plus sibling," so this closes a comparison the copy was already implying.

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice).

**Verified:** `tsc --noEmit` clean (silent = pass). All 3 pages checked live in a local dev server (`/vs/duotts-c29-vs-duotts-duotts-c29max-electric-bike`, `/vs/vtuvia-zeal-lt7-vs-vtuvia-zeal-xt8`, `/vs/samebike-lo26-plus-vs-samebike-lo26-ii-yd`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, zero console errors on any of the three.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (23 vs-pages total now, 5 brand-level + 18 bike-level), each targeting a real "which SKU" query with a genuine spec-driven story. Also closes out this run's two standing-check re-verifications (price-drift, stub-description sweep) with a clean bill of health, confirming no regressions have crept back into either bug class since they were last fixed.

**Next candidates:** (1) GSC/PostHog flat for 16+ consecutive runs; continue leaning on P4.2 slices while watching for a new signal -- Eunorau FLASH LITE ST ticking up to 5 affiliate clicks (from 4) is the only real movement this run, but it already has full depth and a vs-page, so no further action needed there specifically. (2) Remaining same-stem naming-confusion candidates for a future slice: DYU C-series (C2/C5/C6, all `torque=0`, differentiated mainly by wheel size 16"/27.5"/26" -- a weaker story than the pairs shipped so far, lower priority); SAMEBIKE RS-A01 Plus (Pro-vs-Men already paired; Plus is the third variant, worth a look); ENGWE P275 ST vs P275 SE (both step-through, different motor type). (3) The bigger P4.2 location x category page-matrix (new URLs, not a re-sort of existing pages) remains unscoped -- only pursue with a real per-combo differentiation plan, not a blind template. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) Standing checks re-verified clean this run: price-drift (zero drift across 61 rows) and stub-description sweep (zero remaining stubs besides the flagged P0.16b item) -- both worth a periodic re-check, not every run. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 60 (2026-07-29)

**GSC signals:** Flat for a 17th consecutive run -- 2 clicks / 1,730 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/82 impr/pos 39.4). Top pages unchanged: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/38 impr/pos 9.5), `/` (2 clicks/5 impr), `/blog/best-ebikes-for-heavy-riders` (2 clicks/502 impr/pos 29.3), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra, `/blog/best-ebikes-for-hills`. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 158 pageviews / 78 visitors (28-day window), essentially flat vs run 59 (171/83). Top pages unchanged: heavy-riders blog (20/14), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `/` (12/11), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events -- Eunorau FLASH LITE ST holding at 5 clicks, still the clear top converter; DYU M20 and DUOTTS S26 AWD both at 2, DUOTTS F20 at 1. ENGWE N1 Pro (2 clicks in run 59) did not reappear in this run's top-converter list -- worth a second-run check before treating as a real drop rather than window noise. No new signal bikes.

**Action taken:** with both data sources flat for the 17th straight run and every standing data-quality/score-integrity/superlative/price-drift sweep already closed clean, continued run 59's queued next-candidate #2: P4.2's within-brand naming-confusion vs-page slice (4th installment). Pulled fresh specs via Supabase for the candidates run 59 flagged as worth checking:

- **`engwe-p275-se-vs-engwe-p275-st`** ("P275 SE vs P275 ST"): both step-through frames (unlike the already-shipped SE-vs-Pro pairing, which contrasts step-through against step-over), but a genuinely different motor architecture -- SE is rear-hub/55 Nm/$899/46mi practical range, ST is mid-drive/70 Nm/$1,199/121mi practical range. A real "which step-through P275" query for a buyer who already knows they want the low-step frame.
- **`samebike-rs-a01-pro-vs-samebike-rs-a01-plus`** ("RS-A01 Pro vs Plus"): the third RS-A01 variant (already have Pro vs Men from run 15). Both step-through; Plus is a genuine $240 power upgrade (70 Nm/48V 14Ah vs Pro's 55 Nm/36V 15Ah) trading some range (40mi vs 44mi practical) for torque -- a real "is the Plus worth $240 more" query.
- **`dyu-c5-vs-dyu-c6`** ("C5 vs C6"): DYU's closest same-price pair ($799 vs $819), both rear-hub/front-suspension city bikes, differing mainly in frame geometry (C5 step-over 27.5in wheels vs C6 step-through 26in wheels) and battery (10Ah vs 12Ah) -- a real "which frame" query, same pattern as the SX20/SN100 and CREST/STORM pairings. Left `dyu-c2` out of this slice (16in folder, a different form factor rather than a frame/battery tweak -- a weaker naming-confusion story per run 59's own note).

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice). No Supabase writes this run -- all three pairs used specs already correct and web-verified in prior runs (P275 ST/SE data from prior fixes, RS-A01 family from run 15/6, DYU C5/C6 from the run-32 stub-description batch), so this was a page-generation-only slice with a standing-check pre-verification, not a data-fix run.

**Verified:** `tsc --noEmit` clean (silent = pass). All 3 pages checked live in a local dev server (`/vs/engwe-p275-se-vs-engwe-p275-st`, `/vs/samebike-rs-a01-pro-vs-samebike-rs-a01-plus`, `/vs/dyu-c5-27-5-inch-city-electric-bike-vs-dyu-c6-26-inch-city-electric-bike`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, zero console errors on any of the three. Noted that the DYU C5/C6 page displays "Motor: 0 Nm" on both sides -- this reflects the DYU C-series' pre-existing, previously-documented `torque=0` field (DYU does not publish a torque spec for this series; site policy since run 32/33 is to leave it blank rather than fabricate a number), already displayed identically on both bikes' own detail pages, so this is expected behavior carried through to the comparison page, not a new bug.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (26 vs-pages total now, 5 brand-level + 21 bike-level), each targeting a real "which SKU" query with a genuine spec-driven story. Most obvious same-brand naming-confusion pairs are now shipped after 4 slices (runs 55-57, 59-60); future slices will need a closer per-brand scan for remaining unpaired siblings rather than an easy queued list.

**Next candidates:** (1) GSC/PostHog flat for 17+ consecutive runs; watch whether ENGWE N1 Pro's drop from the top-converter list (2 clicks in run 59, absent this run) repeats next run before treating it as a real decline rather than window noise. (2) Remaining P4.2 naming-confusion candidates are thinner now (DYU C2 vs C5/C6, a weaker form-factor-difference story) -- next slice may need a fresh per-brand sweep rather than reusing the run-59 candidate list, which is now exhausted. (3) The bigger location x category page-matrix (new URLs, not a re-sort) remains unscoped -- only pursue with a real per-combo differentiation plan. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy. (5) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 61 (2026-07-29)

**GSC signals:** Flat for an 18th consecutive run -- 2 clicks / 1,730 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/82 impr/pos 39.4). Top pages unchanged: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/38 impr/pos 9.5), `/` (2 clicks/5 impr), `/blog/best-ebikes-for-heavy-riders` (2 clicks/502 impr/pos 29.3), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra, `/blog/best-ebikes-for-hills`. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 158 pageviews / 78 visitors (28-day window), essentially flat vs run 60. Top pages unchanged: heavy-riders blog (20/14), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `/` (12/11), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events -- Eunorau FLASH LITE ST holding at 5 clicks, still the clear top converter; DYU M20 and DUOTTS S26 AWD both at 2, DUOTTS F20 at 1. ENGWE N1 Pro absent from the top-converter list for a 2nd consecutive run (was present run 58, absent runs 60-61) -- this now looks like a real drop-off rather than window noise, but the page already has full data-quality/depth investment (P0.42-0.45) and no new bug turned up, so there's nothing further to action on it this run. No new signal bikes.

**Action taken:** with both data sources flat for the 18th straight run and every standing data-quality/score-integrity/superlative/price-drift sweep already closed clean, and the run-60 P4.2 candidate list exhausted, did a fresh full-catalog per-brand sweep for remaining same-stem naming-confusion sibling pairs (slice 5). Queried every bike's brand/model/price/torque/battery/weight/frame_type via Supabase and identified 3 clean pairs with genuine spec-driven stories:

- **`eunorau-specter-s-hunter-vs-eunorau-specter-st-1`** ("SPECTER-S 3.0 vs SPECTER-ST 2.0"): identical price ($2,999), motor (1000W Bafang M620 mid-drive, 160 Nm), battery (17Ah), and dual-battery range (80mi/60mi practical) -- the only real difference is frame geometry (sport/step-over vs step-through), same pattern as CREST/STORM and SX20/SN100.
- **`vtuvia-reindeer-step-thru-electric-bike-vs-vtuvia-reindeer-2`** ("Reindeer 1.0 vs Reindeer 2.0"): a genuinely counterintuitive upgrade story -- 2.0 is $100 *cheaper* than 1.0 ($1,599 vs $1,699) while adding more torque (85 vs 80 Nm) and a torque sensor, trading off a slightly heavier build (75 vs 72 lbs) and less manufacturer-rated range (70mi vs 55mi... note 2.0's real-world 45mi vs 1.0's 38mi is actually higher, an interesting nuance the page's verdict surfaces via the score).
- **`eunorau-meta-275-1-vs-eunorau-meta-275-st-1`** ("META275 1.0 vs META275 ST"): byte-identical specs (price, motor, torque, battery, weight, range all match exactly) -- the only difference is frame geometry.

**Bug found and fixed while sourcing the META275 pairing (before shipping the vs-page):** `eunorau-meta-275-1`'s `description` and `highlights` both claimed "step-through" ("27.5-inch step-through commuter e-bike... step-through frame") despite its own `frame_type` column already correctly saying `step-over`. Verified via Eunorau's own product page (eunorau-ebike.com/products/meta-275 -- style selector defaults to "Step Over") plus 3 independent retailer listings (zoomelectricbikes.com explicitly "step-over cruiser" for the base model; reallygoodebikes.com and bikehighway.com both label the separate "ST" SKU as the step-through one) -- confirmed Eunorau's naming convention is base=step-over, ST=step-through, so `frame_type` was right and the prose was copy-pasted from the ST sibling. Rewrote `meta-275-1`'s description and highlights to correctly say step-over and cross-reference the ST sibling. Also found `eunorau-meta-275-st-1`'s `model` field was just the bare string "META275" (no "ST" suffix at all), which would have made the new vs-page's own title read as two identically-named bikes -- fixed to "META275 ST" for clarity, consistent with the naming convention used everywhere else on the site (RS-A01 MEN/Plus/Pro, SPECTER-S/ST, etc.).

Added all 3 pairs to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice).

**Verified:** `tsc --noEmit` clean (silent = pass). All 3 pages checked live in a local dev server (`/vs/eunorau-specter-s-hunter-vs-eunorau-specter-st-1`, `/vs/vtuvia-reindeer-step-thru-electric-bike-vs-vtuvia-reindeer-2`, `/vs/eunorau-meta-275-1-vs-eunorau-meta-275-st-1`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, page title on the META275 page correctly updated to show "META275 1.0 vs Eunorau META275 ST" after the model-name fix, zero console errors on any of the three.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (29 vs-pages total now, 5 brand-level + 24 bike-level), each targeting a real "which SKU" query with a genuine spec-driven story, plus fixes a real trust bug (a spec-table field silently contradicted by its own prose) that a routine per-brand sweep surfaced without a dedicated bug hunt.

**Next candidates:** (1) GSC/PostHog flat for 18+ consecutive runs; ENGWE N1 Pro's 2-run absence from the top-converter list now looks like a real decline rather than noise, but the page has no outstanding data bug -- nothing further to action. (2) Remaining P4.2 naming-confusion candidates are thin: DYU C2 vs C5/C6 (weaker form-factor story), ENGWE EP-2 3.0 Boost vs EP-2 Boost/Pro (needs a closer look for a clean 2-bike story), Eunorau FLASH vs FLASH AWD (different motor architecture, verify carefully before pairing) -- future slices should keep verifying specs before writing copy, since that's what caught this run's bug. (3) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 62 (2026-07-29)

**GSC signals:** Flat for a 19th consecutive run -- 2 clicks / 1,730 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/82 impr/pos 39.4). Top pages unchanged: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/38 impr/pos 9.5), `/` (2 clicks/5 impr), `/blog/best-ebikes-for-heavy-riders` (2 clicks/502 impr/pos 29.3), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra, `/blog/best-ebikes-for-hills`. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 158 pageviews / 78 visitors (28-day window), essentially flat vs run 61. Top pages unchanged: heavy-riders blog (20/14), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `/` (12/11), `duotts-duotts-c29-k` (11/7), `duotts-s26` (9/8). 10 `affiliate_link_clicked` events -- Eunorau FLASH LITE ST still the clear top converter at 5 clicks; DYU M20 and DUOTTS S26 AWD both at 2, DUOTTS F20 at 1. ENGWE N1 Pro is now fully absent from the converter list (confirming the run-61 "real drop-off" read) -- page already has full data-quality/depth investment, nothing further to action. No new signal bikes.

**Action taken:** with both data sources flat for the 19th straight run and every standing sweep already closed clean, followed the run-61 queued next candidate: continued P4.2 naming-confusion vs-pages, slice 6. Queried specs for the run-61 candidate list (DYU C2/C5/C6, ENGWE EP-2 family, Eunorau FLASH vs FLASH AWD) via Supabase:

- DYU C2 vs C5/C6 turned out to already be covered -- `dyu-c5-27-5-inch-city-electric-bike-vs-dyu-c6-26-inch-city-electric-bike` shipped in run 60. No new pairing needed here.
- **`engwe-ep-2-boost-vs-engwe-ep-2-pro`** ("EP-2 Boost vs EP-2 Pro"): both are the same 30kg/66lb folding fat-tire platform (55 Nm, 48V 13Ah, 20x4.0 tires, Shimano 7-speed) with near-identical stored specs, so this needed real web verification rather than a DB-only comparison. Sourced ENGWE's own product page (engwe.com/products/engwe-ep-2-pro) plus an independent review (cyclingeu.com/614299): Boost uses a torque sensor with a dedicated "boost mode" button and no throttle (EU/UK Class-1-legal), while Pro pairs a cadence sensor with throttle assist and a smaller 160mm brake rotor (vs Boost's 180mm) -- a genuine "which sensor/assist type do I want" query, not a resolved generational upgrade like the L20 pairs.
- **`eunorau-flash-2-vs-eunorau-flash-awd-1-0`** ("FLASH vs FLASH AWD"): FLASH is a single 1000W motor (220 Nm) built for climbing power, FLASH AWD is dual 750W hub motors (184 Nm combined) built for traction -- a real "climbing power vs all-wheel traction" query, and FLASH AWD is $400 cheaper.

**Bug found and fixed while sourcing the FLASH pairing (before shipping the vs-page):** `eunorau-flash-2`'s `motor_type` was stored as `rear-hub` despite its own `description` explicitly naming a "52V 1000W Truckrun mid-drive motor." Verified via 6+ independent retailer listings (bikeberry.com, ebikejoy.com, offgridlux.com, omahasportsandgames.com, energyebikes.com, offroadebikesource.com), all agreeing the FLASH 2.0 is genuinely mid-drive -- fixed `motor_type` to `mid-drive`. Also checked whether the AWD bikes' shared `motor_type='rear-hub'` value (`eunorau-fat-awd-2/3-0`, `duotts-s26`, `eunorau-flash-awd-1-0`) was the same bug -- confirmed it's intentional, consistent sitewide labeling for dual-hub-motor bikes (no dedicated "AWD" enum value exists), so left untouched.

**New discovery, not yet fixed:** while verifying the FLASH page live, spotted a `motor_brand` column that's populated for only 33 of 109 bikes with a real brand name (21 ENGWE, 11 Bafang, 1 Ananda) -- the other 76 rows are either `NULL` (38) or hold the placeholder value `"Hub"` (37 rows) or `"Mid-drive"` (1 row, itself a motor *type*, not a brand). `eunorau-flash-2` now shows a correct "Motor type: Mid-drive" spec row directly above a nonsensical "Motor brand: Hub" row on the same page. Flagging for a future dedicated sourcing sweep (would need to look up each bike's actual motor manufacturer -- Bafang, Ananda, MIVICE, Truckrun, etc. -- rather than guess) instead of touching it without verification this run.

Added both pairs to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice).

**Verified:** `tsc --noEmit` clean (silent = pass). Both pages checked live in a local dev server (`/vs/engwe-ep-2-boost-vs-engwe-ep-2-pro`, `/vs/eunorau-flash-2-vs-eunorau-flash-awd-1-0`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, zero console errors on either. Also re-checked `/e-bikes/eunorau/eunorau-flash-2` live: spec table now shows "Motor type: Mid-drive," matching the description text.

**Expected impact:** continues P4.2 with 2 more bottom-of-funnel within-brand comparison pages (31 vs-pages total now, 5 brand-level + 26 bike-level), each targeting a real "which SKU/architecture" query with a genuine spec-driven story, plus fixes a real trust bug (a spec-table field silently contradicted by its own prose) that routine pre-vs-page verification surfaced without a dedicated bug hunt.

**Next candidates:** (1) GSC/PostHog flat for 19+ consecutive runs; ENGWE N1 Pro's affiliate-click drop-off is now confirmed rather than noise, but the page has no outstanding data bug -- nothing further to action. (2) Remaining P4.2 naming-confusion candidates are genuinely thin now -- Walfisk has zero within-brand vs-pages and is worth a fresh look; otherwise re-sweep rather than force a marginal pairing. (3) New candidate: the `motor_brand` sitewide placeholder bug (37 rows "Hub", 1 "Mid-drive", 38 NULL, only 33 with a real brand) -- needs a dedicated per-bike sourcing sweep. (4) Standing checks: price-drift (`description` vs `price`) whenever touching pricing copy. (5) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only).

## Run 63 (2026-07-30)

**GSC signals:** Flat for a 20th consecutive run -- 2 clicks / 1,730 impressions (28-day window), CTR 0.1%. Top query unchanged ("electric bike for heavy riders", 2 clicks/82 impr/pos 39.4). Top pages unchanged: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/38 impr/pos 9.5), `/` (2 clicks/5 impr), `/blog/best-ebikes-for-heavy-riders` (2 clicks/502 impr/pos 29.3), plus single-click pages across `/best/folding-ebikes`, DUOTTS C29-K/F20/S26, Eunorau META24 1.0, SAMEBIKE EBE2, Walfisk ET-7 Ultra. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 156 pageviews / 76 visitors (28-day window), essentially flat vs run 62 (158/78). Top pages unchanged: heavy-riders blog (20/14), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `/` (12/11), `duotts-duotts-c29-k` (11/7), `duotts-s26` (8/7). 10 `affiliate_link_clicked` events -- Eunorau FLASH LITE ST still the clear top converter at 5 clicks; DYU M20 and DUOTTS S26 AWD both at 2, DUOTTS F20 at 1. ENGWE N1 Pro remains absent from the converter list (3rd consecutive run). No new signal bikes. 1 `quiz_completed`.

**Action taken (3 items):** with both data sources flat for the 20th straight run and no new signal bikes, worked the two "new discovery" items run 62 flagged, plus one more P4.2 vs-page slice.

1. **`motor_brand` sitewide placeholder bug fixed (P0.63), resolved as a code fix rather than a 76-bike research sourcing sweep.** Run 62 flagged that 37 rows store the placeholder `"Hub"` (a motor *type*, not a brand) and 1 stores `"Mid-drive"` in the `motor_brand` column, with 38 more `NULL`, and only 33 rows (21 ENGWE + 11 Bafang + 1 Ananda) carrying a real sourced brand name. Sourcing 76 bikes' actual motor manufacturers would need a dedicated web-research pass per bike -- too large and too guess-prone for one run, and inconsistent with the site's standing rule never to fabricate specs. Instead traced the actual user-facing symptom: both `app/e-bikes/[brand]/[model]/page.tsx` and the compare tool (`app/e-bikes/vergelijk/VergelijkClient.tsx`) unconditionally render a "Motor brand" spec row, so the 38 `NULL` rows shipped a visibly blank value next to the label (the same "broken-looking field" pattern as P0.50's ENGWE "Power 0" bug) and the 38 placeholder rows shipped a redundant value duplicating the "Motor type" row directly above it. Fixed at the mapping layer: `lib/ebike-data.ts` now normalizes `motor_brand` to `undefined` when it's `NULL`, `"Hub"`, or `"Mid-drive"`; `lib/types.ts`'s `motorBrand` field is now optional; both templates conditionally render the row only when a real brand exists (the compare tool skips the entire "Brand" spec row if no selected bike has one, rather than showing an em-dash column-by-column). The 33 bikes with a real sourced brand are untouched. `tsc --noEmit` clean.
2. **Recurring price-drift bug caught again (P0.64): `eunorau-r1`.** Ran the standing `description ~ '\$[0-9]'` vs `price` sweep across all 61 rows with embedded dollar figures (per the P0.30/P0.48 "standing check" precedent) and found one live drift: `eunorau-r1`'s `price` column is $3,750 but its description still says "At $4,299, it sits among the priciest bikes in the catalog." Cross-checked `CATALOG-SYNC-LOG.md`: the 2026-07-29 weekly vendor sync applied exactly this cut ($4,299 -> $3,750, matched on vendor handle `r1`/title "R1-19", `updated_at` confirms the same day) -- the sync updated the `price` column but, as with every prior instance of this bug class, had no way to touch the hardcoded dollar figure in the free-text description. Fixed the description to say $3,750. Confirmed no other page (blog posts, best-of pages, the R1's own highlights) references the stale figure -- R1 has no vs-page yet, so no comparison page needed a fix either. The rest of the 61-row sweep came back clean, including `eunorau-r1-plus`'s own "$4,499" mention, which still correctly matches its `price`.
3. **New vs-page: Walfisk's first within-brand pairing (P4.2 slice 7).** Followed run 62's queued candidate: Walfisk had zero within-brand vs-pages. Checked all 3 Walfisk SKUs (`WF26` $999, `WF750 UrbanX` $1,499.99, `ET-7 Ultra` $2,299.99) rather than forcing an arbitrary pair. WF26 and WF750 UrbanX share the identical 80 Nm rear-hub motor, front suspension, 330 lb payload and Class 3 rating, differing mainly in battery size (22.5Ah vs 35Ah) for 38mi vs 45mi practical range at $500 more -- a genuine "is the bigger battery worth $500" query, so paired those two. ET-7 Ultra is a 3000W off-road-only motor (not street-legal, per its own existing disclosure) and has no natural same-category pairing with either street-legal sibling, so it was deliberately left out rather than forced into a pairing with a weak story. Added to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean after all three changes (ebike-data.ts, types.ts, page.tsx, VergelijkClient.tsx, sitemap.ts, vs page.tsx). Checked live in a local dev server: `/vs/walfisk-walfisk-26-fat-tire-bafang-750w-powerful-brushless-motor-22-5ah-large-ca-vs-walfisk-walfisk-wf750-urbanx-fat-tire-electric-bike-48v-750w-25ah-battery-up-to-` renders all spec rows correctly (price, score, range, motor, battery, weight, payload), zero console errors; `/e-bikes/eunorau/eunorau-r1` shows the corrected $3,750 in both the price header and body copy; `/e-bikes/duotts/duotts-c29` (motor_brand was NULL) now shows no "Motor brand" row at all; `/e-bikes/engwe/engwe-n1-pro` (real value) still correctly shows "Motor brand: Ananda" -- confirming the fix only hides placeholder/blank values, not real data.

**Expected impact:** closes both "new discovery" items run 62 flagged without fabricating any of the 76 missing motor-brand values, fixes a second live instance of the recurring price-drift bug class on a bike with no other trust-signal issues, and adds Walfisk's first within-brand comparison page (32 vs-pages total now, 5 brand-level + 27 bike-level), closing the last brand with zero internal vs-page coverage.

**Next candidates:** (1) GSC/PostHog flat for 20+ consecutive runs; no new signal bikes, same converter set as the last several runs. (2) Remaining P4.2 naming-confusion candidates are thin across most brands now -- next slice should re-sweep the full catalog for any remaining same-stem sibling pairs rather than reuse an exhausted candidate list. (3) Standing checks: re-run the price-drift sweep (`description ~ '\$[0-9]'` vs `price`) every run, especially after any `CATALOG-SYNC-LOG.md` sync entry touches a bike's price -- this is now the 3rd instance of this exact bug class (runs ~26-27, 43, 63), confirming it recurs on a sync cadence, not a one-time fix. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 64 (2026-07-30)

**GSC signals:** 3 clicks / 1,733 impressions (28-day window), CTR 0.2% -- up from 2 clicks for the first time in 21 runs. Top query still "electric bike for heavy riders" (2 clicks/83 impr/pos 39.2), but a genuinely new query appeared: **"samebike lo26 plus review"** (1 click/4 impr/25.0% CTR/pos 13.0), matching `/e-bikes/samebike/samebike-lo26-plus` (1 click/22 impr/4.5% CTR/pos 14.8). Checked the page: it already has full P0.33 editorial depth (619-char description) and a vs-page (`samebike-lo26-plus-vs-samebike-lo26-ii-yd`, run 59) with an accurate 25% query-level CTR -- not a striking-distance or low-CTR case by the tool's own thresholds, so nothing to fix, just a positive signal worth noting. No other striking-distance queries or high-impression/low-CTR pages surfaced.

**PostHog signals:** 157 pageviews / 77 visitors (28-day window), flat vs run 63 (156/76). Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `/` (12/11), `duotts-duotts-c29-k` (11/7), `duotts-s26` (8/7). `eunorau-fat-awd-3-0` appeared with 3 views/3 visitors (already has full depth from P0.25/P0.40, nothing to action) and `duotts-c29max` logged 4 views/1 visitor (single-visitor refresh pattern, not a real signal -- watch for a 2nd run before treating as new). 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS F20 1). 1 `quiz_completed`.

**Action taken:** ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price` across all 61 rows) -- clean, zero drift this run. With no new fixable signal from GSC/PostHog, continued P4.2's naming-confusion vs-page slice (8th installment) with a fresh full-catalog per-brand sweep for unpaired same-stem siblings (the run-63 candidate list was exhausted). Found 3 genuine pairs that had never been paired despite an obvious shared name stem:

- **`eunorau-r1-vs-eunorau-r1-plus`** ("R1 vs R1+"): the site's two most expensive flagships literally share the "R1"/"R1+" name stem but had somehow never been paired across 7 prior P4.2 slices. R1+ is a real generational upgrade (72V 5,000W/500 Nm vs R1's 72V 4,000W/330 Nm, 150 lbs vs 130 lbs) for $749 more -- a genuine "is the Plus worth it" query between the catalog's two priciest bikes.
- **`engwe-n1-air-vs-engwe-n1-pro`** ("N1 Air vs N1 Pro"): both confirmed carbon fiber frames (run 38 fix) but a real architecture gap -- N1 Air is rear-hub/40 Nm/34.4 lbs (the lightest ENGWE bike) at $1,249, N1 Pro is mid-drive/80 Nm/41.9 lbs with a 1.5hr flash-charge system at $1,599. Both bikes carry real GSC/PostHog signal history from prior runs.
- **`eunorau-meta275-2-vs-eunorau-meta-275-st-1`** ("META275 2.0 vs META275 ST"): while sourcing this pairing, found and fixed a real bug before shipping it -- `eunorau-meta275-2` stored `weight_lbs=60` and `frame_type='step-over'`, but its own description already said "step-thru city commuter." Verified via Eunorau's own product page for this exact SKU (ca.eunorau-ebike.com/products/meta275-2-0): confirms a step-thru frame option and 68.4 lbs, not 60. Fixed `weight_lbs` and `frame_type` in Supabase, then rewrote the now-stale "At 60 lbs, it is one of the lighter bikes in the Eunorau lineup" description sentence (which would have directly contradicted the corrected 68.4 lb spec-table value). With that fixed, 2.0 and ST are both step-through META275s at nearly identical weight (68.4 vs 68.3 lbs), but 2.0 trades 10 Nm of torque (55 vs 65) for a bigger 15Ah battery and much longer range (100mi/75mi vs 65mi/49mi) at $300 more -- same pattern as the P275 SE-vs-ST pairing.

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice).

**Verified:** `tsc --noEmit` clean (silent = pass) after the Supabase fix and both TS file edits. All 3 vs-pages checked live in a local dev server (`/vs/eunorau-r1-vs-eunorau-r1-plus`, `/vs/engwe-n1-air-vs-engwe-n1-pro`, `/vs/eunorau-meta275-2-vs-eunorau-meta-275-st-1`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides, zero console errors on any of the three. Also re-checked `/e-bikes/eunorau/eunorau-meta275-2` live: spec table now shows "Weight: 68.4 lbs" / "Frame type: Step-through", matching both the corrected description and the "consider alternatives" copy that already read from the DB field, zero console errors.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (35 vs-pages total now, 5 brand-level + 30 bike-level), each targeting a real "which SKU" query with a genuine spec-driven story, plus fixes a real trust bug (a spec-table field silently contradicted by its own prose, on a $1,699 bike) that routine pre-vs-page verification surfaced without a dedicated bug hunt -- the 9th time this exact bug class has been caught this way (P0.17/P0.42/P0.45/P0.46/P0.49/P0.54/P0.61/P0.62/P0.65).

**Next candidates:** (1) GSC's first non-flat signal in 20+ runs -- a new "samebike lo26 plus review" query at pos 13/25% CTR -- but the matching page already has full depth and a vs-page, so watch for a 2nd run of this query before treating it as more than a one-off. (2) `duotts-c29max`'s 4-views/1-visitor PostHog blip needs a 2nd-run confirmation before being treated as a real signal (single-visitor refresh pattern looks like noise). (3) Remaining P4.2 naming-confusion candidates are very thin now after 8 slices -- next slice will likely need to look at weaker/more speculative stems (e.g. ENGWE M1 vs M20, VTUVIA CMB vs FMB) or shift toward the bigger unscoped location x category page-matrix instead. (4) Standing checks: price-drift sweep every run (clean this run). (5) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (7) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 65 (2026-07-30)

**GSC signals:** 3 clicks / 1,733 impressions (28-day window), CTR 0.2% -- essentially identical to run 64's same-day pull (same window overlap). Top query still "electric bike for heavy riders" (2 clicks/83 impr/pos 39.2), and the new "samebike lo26 plus review" query (1 click/4 impr/25.0% CTR/pos 13.0) from run 64 repeated unchanged -- since this is the same 28-day window as run 64's pull rather than a distinct later measurement, it does not yet count as "a 2nd run" of that signal; watch the next materially later pull instead. No striking-distance queries, no high-impression/low-CTR pages surfaced.

**PostHog signals:** 158 pageviews / 78 visitors (28-day window), essentially flat vs run 64 (157/77). Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (17/5), `engwe-p275-se` (13/13), `duotts-duotts-c29-k` (11/7), `/` (10/10), `duotts-s26` (8/7). `duotts-c29max` still shows the 4-views/1-visitor blip flagged in run 64 -- same single visitor, no growth, still not a confirmed signal. 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS F20 1). No new signal bikes.

**Action taken:** with both data pulls essentially identical to run 64's same-day numbers (no distinct new signal to act on), ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price` across all 61 rows with embedded dollar figures, including cross-referenced sibling-delta math like "$150 more than the F26" and "$350 more than the N1 Pro") -- manually verified every row, zero drift found, clean. Then continued P4.2's naming-confusion vs-page slice (9th installment) with a fresh full-catalog sweep for remaining unpaired same-stem siblings, cross-checking against all 32 already-shipped MATCHUPS pairs first to avoid duplicates:

- **`eunorau-fat-awd-2-vs-eunorau-fat-awd-3-0`** ("FAT-AWD 2.0 vs 3.0"): identical price ($1,699), motor (dual 500W hub, 110 Nm combined), battery (48V 15Ah) and weight (79.4 lbs) -- both bikes' own descriptions already spell out the real differentiator: 2.0 uses a cadence sensor, 3.0 upgrades to a torque sensor for smoother assist. A genuine "is the torque sensor worth it" query at an identical price point, no new sourcing needed since both descriptions already documented the difference from a prior run.
- **`engwe-m1-vs-engwe-m20`** ("M1 vs M20"): a counterintuitive naming trap -- M1 ($1,099, 65 Nm, 441 lb payload) actually beats M20 ($1,399, 55 Nm, 265 lb payload) on both torque and payload capacity despite the lower model number and lower price. Verified live: on the site's own scoring model M20 still edges out M1 overall (8.1 vs 8.0, mostly on comfort/build axes), so the vs-page's verdict correctly surfaces the "M20 wins overall but M1 wins on raw power/payload for less money" nuance rather than a one-sided story.
- **`engwe-ep-2-3-0-boost-vs-engwe-ep-2-boost`** ("EP-2 3.0 Boost vs EP-2 Boost"): another counterintuitive "3.0" trap, same pattern as the L20/L20 3.0 pairs -- EP-2 3.0 Boost ($999, 75 Nm, torque sensor, hydraulic brakes, magnesium step-through frame) is cheaper than the plain EP-2 Boost ($1,049, 55 Nm, mechanical brakes) while beating it on power. Verified live: 3.0 Boost also wins overall (8.2 vs 7.8) and on value (9.0 vs 8.8), a clean unambiguous case for the "3.0" tier despite the confusing naming.

Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts` (both lists updated together, per established practice).

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). All 3 pages checked live in a local dev server (`/vs/eunorau-fat-awd-2-vs-eunorau-fat-awd-3-0`, `/vs/engwe-m1-vs-engwe-m20`, `/vs/engwe-ep-2-3-0-boost-vs-engwe-ep-2-boost`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted in each verdict, zero console errors on any of the three.

**Expected impact:** continues P4.2 with 3 more bottom-of-funnel within-brand comparison pages (35 vs-pages total now, 5 brand-level + 30 bike-level), each targeting a real "which SKU" query with a genuine, already-documented spec-driven story -- no new web research needed since all three differentiators were already written into the bikes' own descriptions from prior data-quality passes. Re-confirms the price-drift sweep stays clean (3rd consecutive clean run since the run-63 `eunorau-r1` catch).

**Next candidates:** (1) GSC/PostHog pulls were essentially identical to run 64's same-day numbers -- no distinct new signal this run; watch the next materially later pull for whether "samebike lo26 plus review" and the `duotts-c29max` PostHog blip persist or were one-off. (2) Remaining P4.2 naming-confusion candidates are very thin after 9 slices across most brands -- next slice will likely need either a genuinely fresh sweep angle (e.g. cross-brand "similar spec, different brand" pairs, not just within-brand name stems) or a pivot to the bigger unscoped location x category page-matrix (new URLs, not a re-sort) flagged since run 58. (3) Standing checks: price-drift sweep clean for a 3rd consecutive run. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

---

## Run 66 (2026-07-31)

**GSC signals:** 3 clicks / 1,781 impressions (28-day window), CTR 0.2%. Top query still "electric bike for heavy riders" (2 clicks/88 impr/pos 38.8). Top pages: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/45 impr/pos 9.6, already fully optimized), `/blog/best-ebikes-for-heavy-riders` (2 clicks/530 impr/pos 28.8). Two new signals vs run 65: `/best/folding-ebikes` logged its first-ever GSC click (1 click/11 impr/9.1% CTR/pos 21.7) -- every prior run showed 0 clicks on this page -- and the homepage `/` got a click at pos 4.0 (25% CTR). No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 165 pageviews / 75 visitors (28-day window). `/best/folding-ebikes` also showed a PostHog signal this run (4 views/3 visitors/3 sessions) -- a genuine dual-signal convergence with the new GSC click, first time this page has shown up in both sources in the same run. `samebike-xd26-ii` (3 views/2 visitors) is a new single-pull blip, already has a full description from the run-29 SAMEBIKE batch, nothing to fix there. Top converters: Eunorau FLASH LITE ST (5 clicks, highest single-bike count logged yet, already has a vs-page from run 55), DYU M20 (2), DUOTTS S26 (2), DUOTTS C29-K (1), DUOTTS F20 (1) -- 11 total `affiliate_link_clicked` events, 3 `quiz_completed`.

**Work this run (1 item):**

Followed the P4.2 pivot flagged as the run-65 "next candidate": same-stem within-brand vs-pages were "very thin after 9 slices," needing either a cross-brand angle or the location x category matrix. The folding-ebikes dual signal gave a concrete, data-driven reason to try the cross-brand angle there specifically rather than sweep blindly. Queried the catalog for all 12 bikes with a real `dimensions.foldedSize` value: 11 are ENGWE, only `samebike-cy20-pro` is a different brand. Paired it against its closest ENGWE price neighbor, `engwe-t14` ($759/35 Nm/full suspension/330 lb payload vs $650/30 Nm/no suspension/220 lb payload) -- a genuine "is the extra suspension and payload worth $109" cross-brand query, added to `MATCHUPS` (`app/vs/[slug]/page.tsx`) and `VS_MATCHUPS` (`app/sitemap.ts`). Verified both bikes' existing descriptions against the DB first (no data bugs found, both already accurate from prior runs). Ran the standing price-drift sweep (`description ~ '\$[0-9]'` vs `price`, all 61 rows) -- clean, with 2 regex false-positives checked and confirmed genuinely fine (`engwe-l16`'s "under $900" and Walfisk WF750 UrbanX's "$2,000+ price tag" are both price-tier framing, not the bike's own price, and both bikes' actual prices already appear correctly elsewhere in their own descriptions). `tsc --noEmit` clean. Verified live in dev server (`/vs/samebike-cy20-pro-vs-engwe-t14`): correct title, both sides' specs render (price, score, range, motor, battery, weight, payload), correct overall-score winner highlighted (CY20 Pro 6.9 vs T14 6.8), zero console errors.

**Verified:** `tsc --noEmit` clean. Live dev-server check on the new vs-page confirmed spec accuracy and zero console errors.

**Expected impact:** gives the folding-ebikes category's one cross-brand bike a comparison surface exactly as its parent best-of page starts showing its first real dual-signal traffic, and establishes the cross-brand pairing pattern P4.2 will need going forward now that within-brand name-stem pairs are exhausted. 36 vs-pages total now (5 brand-level + 31 bike-level).

**Next candidates:** (1) `/best/folding-ebikes`'s dual-signal event (first GSC click + PostHog signal in the same run) needs a 2nd consecutive run to confirm it's a real trend before further investment beyond the new vs-page. (2) Homepage's first-ever GSC click (pos 4.0) and `samebike-xd26-ii`'s PostHog blip are single-pull signals, same "watch for a 2nd run" treatment. (3) Standing price-drift sweep clean for a 4th consecutive run. (4) Next P4.2 angle: sweep for more cross-brand "same price tier, same category" pairs beyond folding (e.g. budget commuter or cargo bikes across brands) now that same-stem pairs are exhausted, or pivot to the bigger unscoped location x category page-matrix flagged since run 58. (5) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (7) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 67 (2026-07-31)

**GSC signals:** 3 clicks / 1,781 impressions (28-day window), CTR 0.2% -- essentially identical to run 66's same-day pull. Top query still "electric bike for heavy riders" (2 clicks/88 impr/pos 38.8), "samebike lo26 plus review" repeated (1 click/4 impr/25.0% CTR/pos 13.0). `/best/folding-ebikes` (1 click/11 impr/pos 21.7) and the homepage's pos-4.0 click both repeated unchanged from run 66 -- since this is the same overlapping 28-day window rather than a distinct later measurement, neither counts as a confirmed 2nd-run signal yet. No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 143 pageviews / 73 visitors (28-day window), roughly flat vs run 66 (165/75). Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (15/4), `engwe-p275-se` (13/13/13 -- tied for the run's #1 product page), `/` (11/9), `duotts-duotts-c29-k` (7/7), `duotts-s26` (5/5). `samebike-xd26-ii` repeated its 3-views/2-visitors blip from run 66 -- already has a full P0.33-batch description, nothing to fix content-wise. 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). 3 `quiz_completed`.

**Work this run (2 items):**

Both data pulls were essentially identical to run 66's same-day numbers, so ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price`, manually cross-checked all 61 rows including sibling-delta math like "$350 more than the N1 Pro" and "$300 more than the base M20") -- clean, 5th consecutive clean run since the run-63 catch. With no new fixable GSC/PostHog signal, continued the P4.2 cross-brand pivot started in run 66 (same-stem within-brand pairs were exhausted after 9 slices; run 66 shipped the first cross-brand pair off the folding-ebikes dual signal). Queried the catalog for cross-brand twins at identical price points with a genuine spec story:

- **`engwe-p275-se-vs-samebike-rs-a07`** ("P275 SE vs RS-A07"): near-identical cross-brand twins at the exact same $899 price -- both step-through rear-hub commuters with the identical 55 Nm torque figure, differing mainly in payload (P275 SE 265 lb vs RS-A07's meaningfully higher 330 lb) and wheel size (27.5in vs 26in). Gives P275 SE, this run's #1 PostHog page (13 views/13 visitors/13 sessions) and historically the site's top GSC-impression bike page, a fresh comparison surface.
- **`engwe-le20-vs-eunorau-g30-cargo`** ("LE20 vs G30"): the site's first cross-brand cargo-bike comparison. Identical $1,699 price and near-identical payload capacity (441 vs 440 lb), but LE20 clearly out-specs G30 at the same price with more torque (75 vs 65 Nm) and a much bigger battery for nearly double the practical range (81mi vs 49mi) -- a genuine "why pay the same for less range" query.

Both pairings verified against live DB fields before shipping (motor_type, torque, weight_lbs, battery_capacity, range, max_weight) -- no data bugs found, both bike pairs already had accurate specs from prior runs. Added both to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). Both pages checked live in a local dev server (`/vs/engwe-p275-se-vs-samebike-rs-a07`, `/vs/engwe-le20-vs-eunorau-g30-cargo`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted in each verdict, zero console errors on either page.

**Expected impact:** continues the P4.2 cross-brand pivot with 2 more comparison pages (38 vs-pages total now, 5 brand-level + 33 bike-level), one of which targets the run's single strongest PostHog signal bike (P275 SE) and the other opens a wholly new sub-category (cargo bikes) to cross-brand comparison. Re-confirms the price-drift sweep stays clean for a 5th consecutive run.

**Next candidates:** (1) GSC/PostHog pulls were essentially identical to run 66's same-day numbers -- no distinct new signal this run; watch the next materially later pull for whether the folding-ebikes dual signal, `samebike-xd26-ii`'s PostHog blip, or the homepage's pos-4.0 click persist as real trends or were one-off. (2) Standing price-drift sweep clean for a 5th consecutive run. (3) Keep mining cross-brand "same price tier, same category" pairs -- budget folding, fat-tire, and mid-drive city bikes across brands are unexplored angles now that folding and cargo have one pairing each. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 68 (2026-08-01)

**GSC signals:** 3 clicks / 1,813 impressions (28-day window, 2026-07-01 to 2026-07-29), CTR 0.2% -- a materially later pull than run 67's (window shifted forward). Top query still "electric bike for heavy riders" (2 clicks/93 impr/pos 38.8). Two run-67 signals did NOT repeat this pull: the homepage's pos-4.0 click reverted to 0 clicks/pos 7.1 (confirms it was one-off noise, not a trend), and "samebike lo26 plus review" repeated at the exact same 1 click/4 impr/pos 13.0 as runs 64-67 (still the same underlying click, not fresh growth). `/best/folding-ebikes` held its 1 click but impressions grew 11->14 (pos 31.9). Several new zero-click long-tail queries appeared ("300 pound ebike", "50km range commuter e-bike", "affordable commuter ebike") -- nothing actionable yet. No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 145 pageviews / 75 visitors (28-day window), roughly flat vs run 67 (143/73). Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (15/4), `engwe-p275-se` (13/13), `/` (11/9), `duotts-duotts-c29-k` (7/7), `duotts-s26` (5/5). New this run: `eunorau-flash-lite-st` appeared with 5 views/5 visitors -- its first real pageview signal to match its long-standing #1 affiliate-click status (5 clicks); already has full P0.39 editorial depth and a vs-page (run 55), so nothing further to action. `samebike-c05-pro` logged a 2-views/1-visitor blip (single-pull, watch for a 2nd run). `samebike-xd26-ii`'s recurring blip (3 views/2 visitors) continued for a 3rd run, still no fix needed (full P0.33-batch description already in place). 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). No new signal bikes needing action.

**Action taken (2 items):** Both data pulls were essentially flat vs run 67 with no new fixable signal, so ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price`, manually cross-checked all 61 rows including sibling-delta math like "$350 more than N1 Pro" and "$440 over the Pro") -- clean, 6th consecutive clean run since the run-63 `eunorau-r1` catch. Also specifically checked `engwe-l16` since the 2026-07-29 `CATALOG-SYNC-LOG.md` entry (not yet reviewed in any prior run's log) applied a price change to it ($899->$849): its description already correctly says "under $900," still accurate post-change, no fix needed.

With no new fixable GSC/PostHog signal, continued the P4.2 cross-brand vs-page pivot (run 67's queued next candidate) with a fresh full-catalog price/torque/suitable_for cross-tab across all 109 active bikes, looking specifically for cross-brand pairs with a genuine spec story (same-stem within-brand pairs are exhausted after 9 slices, per run 65's log):

- **`samebike-yinyu14-vs-dyu-d3f`** ("YINYU14 vs D3F"): the catalog's two cheapest e-bikes ($449/$499), both ultra-compact 14in folders with identical 30 Nm torque and no suspension. YINYU14 is cheaper with more practical range (24mi vs 20mi); D3F is lighter (41.9 vs 44 lbs) with a higher payload (264 vs 220 lb) -- a real "which budget mini-folder" query.
- **`samebike-sy26-ii-vs-dyu-t1`** ("SY26-II vs T1"): identical $699 price, both front-suspension city commuters. SY26-II wins on torque (45 vs 35 Nm), payload (330 vs 264 lb) and practical range (36mi vs 24mi); T1 counters with a lighter die-cast magnesium frame (50.7 vs 55 lbs) and a torque-sensor pedal feel -- a genuine "same price, which matters more" query.
- **`engwe-e26-vs-vtuvia-reindeer-step-thru-electric-bike`** ("E26 vs Reindeer 1.0"): both exactly $1,699 fat-tire 26in bikes. E26 adds full front-and-rear suspension and more real-world range (65mi vs 38mi); Reindeer 1.0 counters with more torque (80 vs 70 Nm), a much higher payload (400 vs 331 lb) and a lower step-through frame -- a "suspension and range vs torque and payload" cross-brand query at an identical price.

All three pairs verified against live DB fields and each bike's own `description` text before shipping (torque, weight, battery, range, suspension, payload) -- no data bugs found, all descriptions already matched their DB columns cleanly. Added all 3 to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). All 3 pages checked live in a local dev server (`/vs/samebike-yinyu14-vs-dyu-d3f`, `/vs/samebike-sy26-ii-vs-dyu-t1`, `/vs/engwe-e26-vs-vtuvia-reindeer-step-thru-electric-bike`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted in each verdict, zero console errors on any of the three.

**Expected impact:** continues the P4.2 cross-brand pivot with 3 more bottom-of-funnel comparison pages (41 vs-pages total now, 5 brand-level + 36 bike-level), each targeting a genuine "same price/spec tier, which one" cross-brand query mined systematically from a full-catalog cross-tab rather than reusing an exhausted candidate list. Re-confirms the price-drift sweep stays clean for a 6th consecutive run.

**Next candidates:** (1) GSC/PostHog both essentially flat vs run 67 -- the homepage's pos-4.0 click and did-not-repeat "samebike lo26 plus review" growth both confirmed as one-off noise this pull, not real trends. `/best/folding-ebikes` impressions ticked up (11->14) but still only 1 click, not yet a confirmed multi-run trend. Watch `eunorau-flash-lite-st`'s new PostHog pageview signal and `samebike-c05-pro`'s blip for a 2nd run. (2) Standing price-drift sweep clean for a 6th consecutive run; also confirmed the 2026-07-29 sync's `engwe-l16` price change ($899->$849) needed no description fix. (3) Keep mining cross-brand "same price tier, same category" pairs -- the price/torque cross-tab used this run still has untapped candidates (e.g. budget off-road bikes in the $950-1000 band, fat-tire bikes around $1,599-1,699) for a future slice. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 69 (2026-08-01)

**GSC signals:** 3 clicks / 1,813 impressions (28-day window, 2026-07-01 to 2026-07-29) -- byte-identical to run 68's same-day pull (same window, same totals). Top query still "electric bike for heavy riders" (2 clicks/93 impr/pos 38.8). Top pages unchanged: `/e-bikes/samebike/samebike-rs-a01-pro` (2 clicks/50 impr/pos 9.6), `/blog/best-ebikes-for-heavy-riders` (2 clicks/521 impr/pos 28.5), `/best/folding-ebikes` (1 click/14 impr/pos 31.9). No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 145 pageviews / 75 visitors (28-day window) -- identical to run 68's pull. Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (15/4), `engwe-p275-se` (13/13), `/` (11/9), `duotts-duotts-c29-k` (7/7), `duotts-s26` (5/5), `eunorau-flash-lite-st` (5/5, matching its established #1 affiliate-click status, already has full depth + a vs-page). `samebike-c05-pro` repeated its PostHog blip for a 2nd run (2 views/1 visitor) and `samebike-xd26-ii` continued its recurring blip for a 4th run (3 views/2 visitors) -- neither has a vs-page yet despite the repeat signal. 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). No new signal bikes.

**Action taken (2 items):** Both data pulls were byte-identical to run 68's, so ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price`, manually cross-checked all 61 rows including sibling-delta math like "$350 more than N1 Pro," "$300 more than the base M20," and "$50" DUOTTS N26-vs-F26 math) -- clean, 7th consecutive clean run since the run-63 `eunorau-r1` catch.

With no new fixable GSC/PostHog signal, continued the P4.2 cross-brand vs-page pivot with a fresh full-catalog price/torque/suitable_for cross-tab, cross-checked against all 41 existing MATCHUPS pairs to avoid duplicates. Prioritized `samebike-c05-pro` and `samebike-xd26-ii` specifically since both are carrying repeat PostHog blips with no vs-page yet:

- **`samebike-c05-pro-vs-samebike-xd26-ii`** ("C05 Pro vs XD26-II"): same-brand (SAMEBIKE), identical $859 price and 55 Nm torque, the real differentiator is frame (C05 Pro step-through folding vs XD26-II step-over fixed-frame) and range (36mi vs 40mi practical) -- a real "which frame" query, and gives both this run's repeat-blip bikes a comparison surface at once.
- **`eunorau-e-fat-mn-vs-duotts-f20`** ("E-FAT-MN vs F20"): both $1,099 fat-tire folders with a genuine tradeoff -- E-FAT-MN is 80 Nm with no suspension and 37mi practical range, F20 is 70 Nm with full suspension and 56mi practical range -- a real "raw torque vs comfort and range" cross-brand query at the identical price.
- **`vtuvia-giraffe-...-vs-eunorau-defender`** ("Giraffe vs Defender"): both $1,599 with close torque (65 vs 60 Nm) and identical 300 lb payload, but different focus -- Giraffe is a step-through city commuter (50mi manufacturer range), Defender is an AWD hunting/off-road bike (40mi manufacturer range, dual-battery capable) -- a real "commuter vs off-road" cross-brand query at the same price.

**Bug found and fixed while sourcing the E-FAT-MN pairing (before shipping the vs-page):** `eunorau-e-fat-mn`'s description said "up to 40 miles of range" while its own `highlights` array said "Up to 50 mi range" and the DB's `range_manufacturer` column was 50 (with `range_practical`=37) -- a self-contradiction, same bug class as P0.17/P0.42/P0.45/P0.46/P0.49/P0.54/P0.61/P0.62/P0.65. Rewrote the description to state 50mi manufacturer / 37mi practical, matching the DB fields exactly, with no new web research needed since all three figures already existed in the row. First draft of the rewrite added an unverified superlative ("one of the highest torque figures among ENGWE/Eunorau folding fat-tire bikes at this price") -- caught before shipping that this hadn't actually been checked against the rest of the catalog, so removed it rather than repeat the exact false-superlative bug class P0.55-0.58 exist to prevent.

Added all 3 pairs to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass). All 3 pages checked live in a local dev server (`/vs/samebike-c05-pro-vs-samebike-xd26-ii`, `/vs/eunorau-e-fat-mn-vs-duotts-f20`, `/vs/vtuvia-giraffe-step-thru-city-commuter-electric-bike-vs-eunorau-defender`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted in each verdict, zero console errors on any of the three. Also re-checked `/e-bikes/eunorau/eunorau-e-fat-mn` live: description renders the corrected 50mi/37mi range figures, zero console errors.

**Expected impact:** continues the P4.2 cross-brand pivot with 3 more bottom-of-funnel comparison pages (44 vs-pages total now, 5 brand-level + 39 bike-level), one of which directly addresses two bikes with repeat PostHog signal and no prior comparison surface, plus fixes a real description self-contradiction bug (the 10th instance of this exact bug class) caught by routine pre-vs-page spec verification.

**Next candidates:** (1) GSC/PostHog pulls were byte-identical to run 68's same-day numbers -- no distinct new signal this run. Watch `samebike-c05-pro` (2 consecutive PostHog runs now) and `samebike-xd26-ii` (4 consecutive runs) for whether the new vs-page changes their trajectory. (2) Standing price-drift sweep clean for a 7th consecutive run. (3) P4.2 cross-brand candidates are getting thinner -- next slice should check the $1,699 tier's remaining unpaired bikes (`engwe-engine-pro-2-0`, `engwe-engine-pro-3-0-boost`, `eunorau-meta-20-1`, `eunorau-meta-26-1`, `eunorau-max-cargo`, `eunorau-s1-dirt-bike`) for a genuine story, or pivot to the bigger unscoped location x category page-matrix flagged since run 58. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 70 (2026-08-02)

**GSC signals:** 4 clicks / 1,856 impressions (28-day window, 2026-07-02 to 2026-07-30) -- the first clicks-total increase in 20+ consecutive runs (every prior pull since ~run 50 held flat at 2-3 clicks). The new click comes from a brand-new query, **"duotts c29-k"** (1 click/3 impr/33.3% CTR/pos 10.7), matching `/e-bikes/duotts/duotts-duotts-c29-k` (2 clicks total/42 impr/pos 9.9). Top query still "electric bike for heavy riders" (2 clicks/98 impr/pos 38.9). "samebike lo26 plus review" repeated at its established 1 click/4 impr/pos 13.0. No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 146 pageviews / 76 visitors (28-day window), roughly flat vs run 69 (145/75). Top pages unchanged: heavy-riders blog (21/15), `/e-bikes/overzicht` (15/4), `engwe-p275-se` (13/13), `/` (11/9), `duotts-duotts-c29-k` (7/7, a recurring top page for many runs now converging with the new GSC signal), `duotts-s26` (5/5), `/e-bikes/quiz` (5/5), `eunorau-flash-lite-st` (5/5). `samebike-c05-pro`/`samebike-xd26-ii` (both got a vs-page in run 69) did not reappear in the top-20 list this pull. 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). No new signal bikes.

**Action taken (2 items):** The GSC uptick was this run's one genuinely new signal, so checked it first: `duotts-duotts-c29-k` already has a vs-page (`duotts-duotts-c29-k-vs-eunorau-meta-24-1`, run 55) and full P1.1 editorial depth from a prior run, so there was nothing left to fix on that page -- the click increase is organic momentum on already-optimized content, not a bug to catch. Ran the standing price-drift sweep next (`description ~ '\$[0-9]'` vs `price`, all 61 rows, including sibling-delta math like "$350 more than N1 Pro," "$300 more than the base M20," and "$440 over the Pro") -- clean, 8th consecutive clean run since the run-63 `eunorau-r1` catch.

With no further fixable signal, worked run 69's queued $1,699-tier P4.2 candidates directly (`engwe-engine-pro-2-0`, `engwe-engine-pro-3-0-boost`, `eunorau-meta-20-1`, `eunorau-meta-26-1`, `eunorau-max-cargo`, `eunorau-s1-dirt-bike`) rather than a blind fresh sweep, since that list already had genuine same-price stories queued:

- **`eunorau-meta-20-1-vs-eunorau-meta-26-1`** ("META20 1.0 vs META26 1.0"): identical price ($1,699), motor (48V 500W hub, 55 Nm), battery (15Ah) and range (100mi manufacturer/75mi practical) -- the only real difference is wheel size and the rider-height guidance each description already carries (20in for 5'1"-6'1" vs 26in for 5'4"-6'3"), a genuine "which size fits me" query within Eunorau's own META family.
- **`engwe-engine-pro-2-0-vs-engwe-engine-pro-3-0-boost`** ("Engine Pro 2.0 vs Engine Pro 3.0 Boost"): identical price ($1,699) and payload (331 lb), but 3.0 Boost is a real generational upgrade (90 vs 75 Nm, 60mi vs 51mi practical range, hydraulic dual-piston brakes plus GPS/4G anti-theft) for 6.8 lbs more and one fewer gear (7-speed vs 2.0's 8-speed) -- a genuine "is the newer generation worth the tradeoff" query.

`eunorau-s1-dirt-bike` (a 150 Nm mid-drive dirt bike, structurally unlike anything else at this price) and `eunorau-max-cargo` (a cargo bike with no unpaired same-price cargo sibling this run) were left out rather than forced into a weak pairing -- both remain candidates for a future slice if a genuine match turns up. Both shipped pairs verified spec-clean against live DB fields before writing copy (torque, weight, battery, range, payload) -- no data bugs found, both bike pairs already had accurate, consistent descriptions from prior runs. Added both to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). Both pages checked live in a local dev server (`/vs/eunorau-meta-20-1-vs-eunorau-meta-26-1`, `/vs/engwe-engine-pro-2-0-vs-engwe-engine-pro-3-0-boost`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right winner highlighted in each verdict, zero console errors on either page.

**Expected impact:** continues the P4.2 cross-brand/within-brand pivot with 2 more bottom-of-funnel comparison pages (46 vs-pages total now, 5 brand-level + 41 bike-level), each targeting a genuine "same price tier, which one" query mined from run 69's own queued candidate list rather than a fresh blind sweep. Re-confirms the price-drift sweep stays clean for an 8th consecutive run, and confirms the site's first GSC clicks-total increase in 20+ runs traces to organic growth on an already-fully-optimized page rather than a fixable gap.

**Next candidates:** (1) The "duotts c29-k" query and the clicks-total uptick (3->4) need a 2nd consecutive run to confirm it's a real trend rather than a one-off, same treatment as every prior single-pull signal. (2) Standing price-drift sweep clean for an 8th consecutive run. (3) `eunorau-s1-dirt-bike` and `eunorau-max-cargo` remain unpaired from this run's $1,699-tier sweep -- worth a fresh look if a genuine same-price sibling turns up (e.g. another dirt/off-road bike for S1, another cargo bike for MAX-CARGO) rather than forcing a weak match. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 71 (2026-08-02)

**GSC signals:** 4 clicks / 1,856 impressions (28-day window, 2026-07-02 to 2026-07-30) -- byte-identical to run 70's same-day pull. Top query still "electric bike for heavy riders" (2 clicks/98 impr/pos 38.9), "duotts c29-k" and "samebike lo26 plus review" both repeated unchanged from run 70. No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 147 pageviews / 79 visitors (28-day window), roughly flat vs run 70 (146/76). Top pages unchanged: heavy-riders blog (21/15), `engwe-p275-se` (13/13), `/e-bikes/overzicht` (13/4), `/` (12/10), `duotts-duotts-c29-k` (7/7), `/e-bikes/quiz` (5/5), `eunorau-flash-lite-st` (5/5), `duotts-s26` (5/5). 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). No new signal bikes.

**Action taken (2 items):** Both data pulls were byte-identical to run 70's, so ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price`, all 61 rows) -- this time it caught a real bug: `eunorau-flash-lite-2-0` and `eunorau-flash-lite-st` are both priced at exactly $1,899, yet each bike's own description independently claimed to "undercut" the other on price ("it undercuts the similarly-specced FLASH LITE ST" / "it undercuts the sibling FLASH LITE 2.0 on price") -- a mutual contradiction that can't both be true, same bug class as the P0.55 dueling-superlative catch but on a price claim instead of a spec claim, and on the site's single highest-converting bike (FLASH LITE ST, 5 affiliate clicks, the most of any bike logged) and its direct sibling. Rewrote both descriptions to state they're identically priced and differentiate on the real tradeoff (2.0's expandable battery system vs ST's step-through frame) instead of a false price claim.

With that fixed, continued the P4.2 vs-page pivot with a fresh full-catalog sweep for same-stem pairs not yet in the 46-entry `MATCHUPS` list. Found `eunorau-fat-hd`/`eunorau-fat-hd-2-0` ("FAT-HD 1.0 vs 2.0"): same 1000W/160 Nm Bafang mid-drive platform and 375 lb payload, but 2.0 adds a dual-battery system (60mi practical range vs 1.0's 45mi) and front suspension (1.0 has none) for $400 more -- a genuine "is the 2.0 upgrade worth it" query, the same pattern as the already-shipped FAT-AWD 2.0/3.0 pairing, and one that had never been paired in 10 prior P4.2 slices despite an obvious shared name stem. Checked `eunorau-max-cargo`/`eunorau-s1-dirt-bike` again (both unpaired since run 70's $1,699-tier sweep) -- still no genuine same-price sibling (cargo bike vs. mid-drive dirt bike is too different a use case to force), left unpaired by design. Added the FAT-HD pair to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass). Checked live in a local dev server: `/vs/eunorau-fat-hd-vs-eunorau-fat-hd-2-0` renders both sides' specs correctly (price, score, range, motor, battery, weight, payload) with FAT-HD 2.0 correctly highlighted as the higher-scoring pick (8.0 vs 7.3); `/e-bikes/eunorau/eunorau-flash-lite-2-0` and `/e-bikes/eunorau/eunorau-flash-lite-st` both render the corrected same-price framing. Zero console errors on all three pages.

**Expected impact:** fixes a real mutual price-contradiction bug on the site's top-converting bike and its sibling (the kind of self-contradiction a buyer comparing the two pages side by side would notice immediately), and adds one more bottom-of-funnel comparison page mined from a genuine, previously-unpaired same-stem sibling story. 47 vs-pages total now (5 brand-level + 42 bike-level).

## Run 72 (2026-08-03)

**GSC signals:** 4 clicks / 1,918 impressions (28-day window, 2026-07-03 to 2026-07-31) -- a materially later pull than run 71's, impressions up slightly (1,856->1,918) with clicks flat. Top query still "electric bike for heavy riders" (2 clicks/103 impr/pos 39.0). "duotts c29-k" repeated at the same 1 click/3 impr/pos 10.7 as run 70/71 -- not fresh growth, the same underlying click. "samebike lo26 plus review" also repeated unchanged (1 click/4 impr/pos 13.0). No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 149 pageviews / 81 visitors (28-day window), roughly flat vs run 71 (147/79). Top pages unchanged: heavy-riders blog (23/17), `engwe-p275-se` (13/13), `/e-bikes/overzicht` (13/4), `/` (12/10), `duotts-duotts-c29-k` (7/7), `duotts-s26` (5/5), `/e-bikes/quiz` (5/5), `eunorau-flash-lite-st` (5/5). 11 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1, DUOTTS F20 1). No new signal bikes.

**Action taken (1 item):** Both data pulls were essentially identical to run 71's, so ran the standing price-drift sweep first (`description ~ '\$[0-9]'` vs `price`, all 61 rows, including sibling-delta math like "$350 more than N1 Pro," "$300 more than base M20," "$440 over the Pro," and the DUOTTS F26/N26 "$50" comparison) -- clean, the 1st clean run since run 71 caught the FLASH LITE mutual-price-contradiction bug.

With no new fixable GSC/PostHog signal, worked run 71's queued DYU candidate list directly (`dyu-a1f-pro`, `dyu-a5`, `dyu-c2`, `dyu-stroll-1`, all unpaired in the 47-entry `MATCHUPS` list):

- **`dyu-a1f-pro-vs-dyu-a5`** ("A1F Pro vs A5"): identical 25 Nm torque and near-identical folding city-bike builds, but A5 adds a front suspension fork (A1F Pro has none) for $90 more ($519 vs $429) -- a genuine "is the suspension worth $90" query within DYU's own budget folding lineup.
- **`dyu-stroll-1-vs-samebike-lo26-plus`** ("Stroll 1 vs LO26 Plus"): identical $999 price but opposite use cases -- Stroll 1 is DYU's lightest, most road-bike-like model (43 lbs, no suspension, 40 Nm, 700C wheels) built for stylish paved commuting, LO26 Plus is a full-suspension 26x4.0 fat-tire off-roader (68 lbs, 75 Nm, 330 lb payload) -- a real cross-brand "commuter vs off-road" query at the identical price.

`dyu-c2` (torque still not publicly documented, the same standing gap as the C2/C5/C6 trio since run 32) had no genuine same-price sibling this run -- checked bikes priced $500-600 sitewide (`dyu-a5` $519, `engwe-y400` $599) and found no clean story: Y400 is a seated electric scooter (per the P0.13 reclassification), not a real bike-vs-bike comparison. Left `dyu-c2` unpaired by design rather than force a weak or misleading match, same discipline as `eunorau-max-cargo`/`eunorau-s1-dirt-bike`. Both shipped pairs' descriptions verified against live DB fields before writing copy (torque, weight, suspension, battery, range, payload) -- no data bugs found, both pairs already had accurate, consistent descriptions from prior runs. Added both to `MATCHUPS` in `app/vs/[slug]/page.tsx` and the matching entries in `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). Both pages checked live in a local dev server (`/vs/dyu-a1f-pro-vs-dyu-a5`, `/vs/dyu-stroll-1-vs-samebike-lo26-plus`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right verdict, zero console errors on either page.

**Expected impact:** continues the P4.2 vs-page pivot with 2 more bottom-of-funnel comparison pages (49 vs-pages total now, 5 brand-level + 44 bike-level), mined directly from the prior run's own queued candidate list rather than a blind fresh sweep, and keeps the "leave it unpaired rather than force a weak match" discipline intact for `dyu-c2`. Re-confirms the price-drift sweep is clean again after last run's catch.

**Next candidates:** (1) GSC/PostHog pulls were essentially identical to run 71's same-day numbers -- no distinct new signal this run. Watch "duotts c29-k" and "samebike lo26 plus review" for whether they ever grow beyond their established flat click counts. (2) Standing price-drift sweep clean for the 1st run since the run-71 FLASH LITE catch. (3) P4.2 candidates continue to thin out -- `dyu-c2` remains unpaired (no genuine same-price sibling), and within-brand/cross-brand same-stem pairs are largely exhausted across ENGWE, Eunorau, DUOTTS, SAMEBIKE, and now DYU; next slice should check VTUVIA's and Walfisk's remaining unpaired SKUs, or pivot to the bigger unscoped location x category page-matrix flagged since run 58. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

**Next candidates:** (1) GSC/PostHog pulls were byte-identical to run 70's same-day numbers -- no distinct new signal this run. (2) Standing price-drift sweep caught its first bug since the run-63 `eunorau-r1` catch (8 consecutive clean runs before this one) -- keep it a standing per-run check. (3) `eunorau-max-cargo` and `eunorau-s1-dirt-bike` remain unpaired -- worth a fresh look only if a genuine same-price sibling turns up. (4) Remaining P4.2 candidates continue to thin out; next slice should check DYU's 4 still-unpaired budget bikes (`dyu-a1f-pro`, `dyu-a5`, `dyu-c2`, `dyu-stroll-1`) or pivot to the bigger unscoped location x category page-matrix flagged since run 58. (5) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (6) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (7) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.

## Run 73 (2026-08-05)

**GSC signals:** 5 clicks / 1,992 impressions (28-day window, 2026-07-05 to 2026-08-02) -- a materially later pull than run 72's (1,918 impr), with clicks up one (4->5). "electric bike for heavy riders" ticked to 3 clicks/106 impr/pos 38.9 (up from 2), still the authority-ceilinged heavy-riders post, not on-page fixable. "duotts c29-k" repeated at pos 9.5 (was 10.7) and "samebike lo26 plus review" at pos 12.0 (was 13.0), both minor position improvements on the same underlying clicks, not fresh growth. The real new signal: `samebike-ebe2` logged its first-ever confirmed GSC click this run (pos 8.8, page 1, 42 impr, 2.4% CTR) -- no vs-page existed for it. No striking-distance queries, no high-impression/low-CTR pages.

**PostHog signals:** 155 pageviews / 87 visitors (28-day window), up from run 72's 149/81. Top pages unchanged: heavy-riders blog (25/19), `engwe-p275-se` (13/13), `/e-bikes/overzicht` (13/4), `/` (12/10), `duotts-duotts-c29-k` (7/7), `/best/folding-ebikes` (5/4), `/e-bikes/quiz` (5/5), `eunorau-flash-lite-st` (5/5), `duotts-s26` (5/5). New blips this run: `engwe-le20` (4/2, already has a vs-page from run 67 and full P0.42 editorial depth -- nothing further to action), `eunorau-fat-awd-3-0` (3/3), `dyu-m20` (3/1), `eunorau-g30-cargo` (2/2, already paired via the LE20 vs-page), `engwe-ep-2-pro` (2/2, already has a vs-page from run 62). 10 `affiliate_link_clicked` events, same converter set (Eunorau FLASH LITE ST 5, DYU M20 2, DUOTTS S26 AWD 2, DUOTTS C29-K 1), no new signal bikes on the conversion side.

**Action taken (2 items):** `samebike-ebe2`'s first-ever GSC click was this run's one genuinely new, actionable signal, so checked it first: no existing vs-page. Its closest genuine sibling is `samebike-sy26-ii` -- same SAMEBIKE 45 Nm/13Ah rear-hub platform, near-identical spec sheet (26in wheels, Aluminum frame, Shimano 7-speed, front suspension), but a real, already-documented differentiator in both bikes' own existing copy: EBE2 uses a torque sensor and mountain-bike geometry tuned for fire roads/light singletrack, SY26-II uses commuter-tuned geometry with no torque-sensor claim, at a genuine $80 price gap ($619 vs $699). Added the pairing.

Ran the standing price-drift sweep next (`description ~ '\$[0-9]'` vs `price`, all 61 rows, including sibling-delta math like "$350 more than N1 Pro," "$300 more than base M20," "$440 over the Pro," and "undercuts... by $150/$50") -- clean, 2nd consecutive clean run since the run-71 FLASH LITE catch.

With that clean, worked run 72's queued next-candidate #3 (check VTUVIA's remaining unpaired SKUs): of VTUVIA's 10 SKUs, `sx20`/`sn100`, `zeal-lt7`/`zeal-xt8`, `reindeer-1`/`reindeer-2` were already paired within-brand, and `giraffe`/`reindeer-1` were already paired cross-brand (runs 68-69) -- leaving `cmb`, `sf20h`, and `gemini` unpaired. Found a genuine same-price story: `vtuvia-cmb` and `vtuvia-gemini` are both $1,899, VTUVIA's two flagship bikes, never paired despite the obvious "which flagship" query -- CMB is a 175 Nm mid-drive step-through (VTUVIA's highest torque figure) built for climbing efficiency at 60 lbs/300 lb payload, Gemini is a dual-battery 100 Nm rear-hub step-over built for range and cargo capacity at 98 lbs/400 lb payload. `sf20h` ($1,399 folding cargo fat-tire) had no clean same-price match against either remaining bike this run -- left unpaired, queued for a future slice. Both shipped pairs verified spec-clean against live DB fields (torque, weight, battery, range, payload, suspension) before writing copy -- no data bugs found. Added both to `MATCHUPS` in `app/vs/[slug]/page.tsx` and `VS_MATCHUPS` in `app/sitemap.ts`.

**Verified:** `tsc --noEmit` clean (silent = pass, DB reads only, no Supabase writes this run). Both pages checked live in a local dev server (`/vs/samebike-ebe2-vs-samebike-sy26-ii`, `/vs/vtuvia-cmb-vs-vtuvia-gemini`) -- every spec row (price, score, range, motor, battery, weight, payload) renders correctly on both sides with the right verdict, zero console errors on either page.

**Expected impact:** continues the P4.2 vs-page pivot with 2 more bottom-of-funnel comparison pages (51 vs-pages total now, 5 brand-level + 46 bike-level), one directly targeting this run's strongest new signal bike (EBE2's first page-1 GSC position) and the other closing out VTUVIA's flagship-tier gap. Re-confirms the price-drift sweep stays clean for a 2nd consecutive run.

**Next candidates:** (1) Watch `samebike-ebe2` for a 2nd-run repeat of its GSC click before treating it as a confirmed trend rather than a single-pull signal. "electric bike for heavy riders" ticked up to 3 clicks this run -- also watch for a 2nd-run confirmation. (2) Standing price-drift sweep clean for a 2nd consecutive run. (3) `vtuvia-sf20h` is VTUVIA's last unpaired SKU -- worth a fresh look once a genuine same-price sibling turns up elsewhere in the catalog (cross-brand candidates at ~$1,399 are worth checking next). `dyu-c2`, `eunorau-max-cargo`, `eunorau-s1-dirt-bike` remain unpaired by design. Remaining P4.2 candidates continue to thin out; next slice will likely need a pivot to the bigger unscoped location x category page-matrix flagged since run 58. (4) `eunorau-defender-s-fat-hs` (P0.16b) and `samebike-cy20-pro` torque mismatch (P0.28) remain the only two genuinely ambiguous data questions still needing a Dylan/human call. (5) `/best/cargo-ebikes` and the heavy-riders blog post remain authority-ceilinged, not on-page fixable (P4.1, Dylan-only). (6) P3.3 (deals/price-drop page) still blocked on missing original-price/MSRP data in the catalog.
