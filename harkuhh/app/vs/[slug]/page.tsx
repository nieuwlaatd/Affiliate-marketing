...
const MATCHUPS: [string, string][] = [
  ...
  // 2026-07-30 run 65: P4.2 naming-confusion slice 9 -- GSC/PostHog essentially unchanged
  // from run 64's same-day pull (3 clicks/1,733 impr; "samebike lo26 plus review" query
  // repeated with no 2nd-run growth yet), so continued mining the full catalog for
  // remaining same-stem pairs with a real, already-documented differentiator in their
  // existing descriptions (no new web research needed for any of the three below).
  ...
  // 2026-08-01 run 69: GSC/PostHog pulls were essentially identical to run 68's same-day
  // numbers (no distinct new signal), so ran the standing price-drift sweep (clean, 7th
  // consecutive) then continued mining the full-catalog price/torque cross-tab for
  // pairs not yet covered by any of the 41 existing MATCHUPS entries.
  ['samebike-c05-pro', 'samebike-xd26-ii'], // "C05 Pro vs XD26-II" -- both SAMEBIKE, identical $859 price and 55 Nm torque, differing mainly in frame (C05 Pro step-through folding vs XD26-II step-over fixed-frame) and range (36mi vs 40mi practical) -- a real "which frame" query, and both bikes logged a PostHog pageview blip this run (XD26-II's 3rd consecutive, C05 Pro's 2nd) with no vs-page yet
  ['eunorau-e-fat-mn', 'duotts-f20'], // "E-FAT-MN vs F20" -- both $1,099 fat-tire folders, but a genuine suspension/range tradeoff: E-FAT-MN is 80 Nm with no suspension and 37mi practical range, F20 is 70 Nm with full suspension and 56mi practical range -- a real "raw torque vs comfort and range" cross-brand query at the identical price. While sourcing this pairing, found and fixed a bug: E-FAT-MN's description said "up to 40 miles of range" while its own `highlights` said "Up to 50 mi range" and the DB's `range_manufacturer` was 50 -- corrected the description to state 50mi manufacturer / 37mi practical, matching the DB fields (see SEO-LOG)
  ['vtuvia-giraffe-step-thru-city-commuter-electric-bike', 'eunorau-defender'], // "Giraffe vs Defender" -- both $1,599, close torque (65 vs 60 Nm) and identical 300 lb payload, but different focus: Giraffe is a step-through city commuter (50mi manufacturer range), Defender is an AWD hunting/off-road bike (40mi manufacturer range, dual-battery capable) -- a real "commuter vs off-road" cross-brand query at the same price point
  // 2026-08-02 run 70: GSC logged its first clicks-total increase in 20+ runs (3->4),
  // driven by a brand-new "duotts c29-k" query (pos 10.7) matching a page that's also a
  // recurring PostHog top performer -- checked it already has a vs-page (run 55) and full
  // editorial depth, so no fix needed there. Continued the P4.2 cross-brand pivot by
  ...
  // 2026-08-05 run 73: PostHog/GSC flagged `samebike-ebe2`'s first-ever confirmed GSC
  // click this run (pos 8.8, page 1) with no vs-page yet -- this run's strongest signal.
  ['samebike-ebe2', 'samebike-sy26-ii'], // "EBE2 vs SY26-II" -- same SAMEBIKE 45 Nm/13Ah rear-hub platform and near-identical spec sheet, but a real, already-documented sensor/geometry split: EBE2 uses a torque sensor and mountain-bike geometry tuned for fire roads and light singletrack, SY26-II uses a cadence-driven setup with commuter-tuned geometry for pavement -- a genuine "which SAMEBIKE 45Nm bike" query for a buyer who isn't sure whether they want the off-road or street version, at a real $80 price gap ($619 vs $699)
  ['vtuvia-cmb', 'vtuvia-gemini'], // "CMB vs Gemini" -- VTUVIA's two flagship bikes, identical $1,899 price, never paired despite an obvious "which flagship" story: CMB is a 175 Nm mid-drive step-through (VTUVIA's highest torque figure) built for climbing efficiency and easy mounting at 60 lbs/300 lb payload, Gemini is a dual-battery 100 Nm rear-hub step-over built for range and cargo capacity at 98 lbs/400 lb payload -- a real "climbing power vs range and payload" query between the brand's two top-tier bikes
];
